...
| B-112 | Omni parity: expand Admin modules for AI, agents, groups, roles, business hours, portals, email, widgets, phone, chat, feedback form, WhatsApp, ticket forms, automations, email notifications, CSAT surveys, proactive outreach, routing, canned responses, ticket templates, scenario automations, canned forms, tags, threads, and security | P0 | Done |
| B-113 | Omni parity: add support operations surfaces for forums, WhatsApp proactive campaigns, custom objects/purchase history, contact/company fields, multiple products, advanced ticketing, field service scheduling, account exports, scheduled exports, and helpdesk branding settings | P0 | In Progress |
| B-114 | Omni parity: add full Analytics report-library parity beyond current Insights rollups, including report catalog/navigation, saved reports, and export/scheduled report controls | P0 | Done |
...
| C-107 | B-112 admin-module wave (Claude branch): admin catalog tiles are real navigation (no dead toasts), plus ten new full-stack, individually-committed admin modules with real backend tables, Alembic migrations 0026-0034, seeds, and Setup panels — Business hours, Canned responses, Ticket templates, Tags, CSAT surveys, Email notifications, Scenario automations, Contact/Company custom fields, Custom objects, and Products; each verified with ruff/mypy/pytest/alembic + frontend tsc/eslint/build | 2026-06-06 |
| C-108 | B-114 done: replaced the hardcoded analytics saved/scheduled report catalog with a real saved_reports backend (Alembic 0035, seeded reports), CRUD store actions, and an Analytics report library where Saved/Scheduled tabs render live saved reports with a create form, per-report cadence scheduling, pause/activate, and a working CSV export of the live analytics rollups; backend 136 pytest + frontend build pass | 2026-06-06 |
| C-110 | Parity completion wave (Claude branch): closed B-112 (all admin/channel tiles now render and route to real surfaces — Phone, Omnichat, WhatsApp, Facebook, Feedback form, Widgets, Proactive outreach, Profile settings, Security controls, Marketplace apps, Account/Scheduled exports, Field service scheduling, Threads, Canned forms) and advanced B-113. New full-stack, DB-seeded slices, each individually committed and verified (ruff/mypy/pytest + alembic + frontend tsc/eslint/build): Community forums (discussion topics + comments, Alembic 0039), Portal/helpdesk branding (workspace_settings columns, Alembic 0038, applied to the public help center), and Chat widget configuration (Alembic 0040). Also fixed create_schema's legacy-upgrade path to add the new branding columns to pre-existing DBs. Backend 142 pytest. Deferred (no clear spec / no backend): B-113 "Advanced ticketing" and "Purchase history" tiles remain hidden (not dead links) | 2026-06-07 |
| C-109 | Stub-closure wave (Claude branch): (1) B-113 field service scheduling — new service_appointments backend (Alembic 0036), seeds, /service-appointments routes, and a live Workforce dispatch board with a scheduling form, status workflow, and technician reassignment, replacing the mocked schedule grid; (2) Knowledge base screen rewritten from a hardcoded category catalog to live article CRUD (search, create, draft/review/publish) on the existing /knowledge API; (3) Customer 360 gained create + inline edit on the /customers API; (4) Omnichat console module views (Dashboard/Campaigns/People/Reports/Marketplace/Settings/AI Studio) now render live state instead of static capability cards; (5) all credential management centralized in Settings — new DB-backed, admin-editable Enterprise SSO/OIDC settings (Alembic 0037) with environment fallback, alongside the existing SMTP and provider-secret forms; (6) removed remaining dead-link placeholders (dismissable announcement, real user menu, working Omnichat status filter). Backend 138 pytest + ruff/mypy + alembic head; frontend tsc/eslint/build all green; dead CSS cleaned up as work proceeded | 2026-06-06 |
