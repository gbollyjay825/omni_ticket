...
class FacebookWebhookEvent:
    ...
    receipt: FacebookDeliveryReceipt | None = None


class FacebookWebhookAdapter:
    def webhook_external_id(self, payload: dict[str, Any], delivery_id: str | None = None) -> str:
        ...
        if delivery_id:
            return f"facebook:webhook:{delivery_id}"
        body = json.dumps(payload, sort_keys=True, default=str)
        return f"facebook:webhook:{hashlib.sha256(body.encode()).hexdigest()[:24]}"
            ...
            for entry in entries:
                ...
                for event in messaging_events:
                    if isinstance(event, dict):
                        return event
        if any(key in payload for key in ("message", "postback", "delivery", "read")):
            return payload
        return None
    ...
    def _receipt(self, payload: dict[str, Any]) -> FacebookDeliveryReceipt:
        ...
        return FacebookDeliveryReceipt(
            status=status_value,
    ...
    def _inbound(
        ...
        event = self._first_messaging(payload) or payload
        message_payload = self._message_payload(event) or payload
        handle = self._sender_id(event) or self._first_text(payload, "from", "sender", "psid", "handle") or ""
        ...
        customer_name = (
            ...
            or handle
            or "Facebook Messenger Customer"
        )
        metadata_payload = {
            **metadata,
            "facebook_webhook_payload": payload,
            "facebook_event_kind": "inbound",
            "facebook_page_id": self._recipient_id(event),
            "facebook_message_type": "postback" if isinstance(event.get("postback"), dict) else "message",
        }
        return ConnectorInboundRequest(
            ...
            subject=self._first_text(payload, "subject") or f"Facebook Messenger from {customer_name}",
            body=body,
    ...
    def _message_body(self, message: dict[str, Any]) -> str:
        ...
        if isinstance(attachments, list) and attachments:
            return "[Facebook Messenger attachment received]"
        return "[Facebook Messenger message received]"
    ...
    def _synthetic_email(self, value: str) -> str:
        ...
        return f"facebook-{normalized[:80]}@facebook.omniticket.app"
...
facebook_webhook_adapter = FacebookWebhookAdapter()
