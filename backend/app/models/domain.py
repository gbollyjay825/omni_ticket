...
class OidcProviderConfig(BaseModel):
    ...
    password_login_mode: Literal["all", "break_glass", "disabled"]
...
ReportCadence = Literal["none", "daily", "weekly", "monthly"]
...
class AiDecision(BaseModel):
    ...
    input_reference: str = "rules-engine"
...
class CreateTicketViewRequest(BaseModel):
    name: str = Field(min_length=2, max_length=160)
    filters: dict[str, Any] = Field(default_factory=dict)
    sort_by: Literal["updated_at", "created_at", "public_id", "subject", "status", "priority", "channel"] = "updated_at"
    sort_order: Literal["asc", "desc"] = "desc"
    position: int = Field(default=100, ge=0, le=10000)
    shared: bool = False


class UpdateTicketViewRequest(BaseModel):
    name: str | None = Field(default=None, min_length=2, max_length=160)
    filters: dict[str, Any] | None = None
    sort_by: Literal["updated_at", "created_at", "public_id", "subject", "status", "priority", "channel"] | None = None
...
class PersonalWorkspace(BaseModel):
    ...
    watched_ticket_ids: list[str] = Field(default_factory=list)
...
class CreateTicketTimeEntryRequest(BaseModel):
