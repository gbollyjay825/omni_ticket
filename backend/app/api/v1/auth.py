...
def _browser_session(response: Response, session: AuthSession) -> AuthSession:
    _set_browser_session_cookie(response, session.access_token)
    return session.model_copy(update={"access_token": ""})
...
def _password_login_denial_reason(email: str) -> str | None:
    if settings.password_login_mode == "disabled":
        return "password_login_disabled"
    if settings.password_login_mode == "break_glass":
        allowed = {
            value.strip().lower()
            for value in settings.break_glass_admin_emails
            if value.strip()
        }
        if email not in allowed:
            return "break_glass_identity_not_allowed"
    return None
...
def login(
    ...
    password_denial_reason = _password_login_denial_reason(normalized_email)
    if password_denial_reason:
        write_audit_event(
            db,
        ...
        raise HTTPException(
            ...
            detail="Password login is not available for this identity.",
        )
    ...
    if settings.password_login_mode == "break_glass" and (
        user_record.role != UserRole.admin.value or not user_record.mfa_enabled
    ):
        write_audit_event(
            db,
            actor=user_record.id,
            ...
            entity_type="user",
            entity_id=user_record.id,
            market_id=request.market_id or user_record.default_market_id,
            details={
                "email": normalized_email,
                "reason": "break_glass_requires_admin_mfa",
                "requested_market_id": request.market_id,
            },
            commit=True,
        )
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            detail="Break-glass login requires an approved administrator with MFA enabled.",
        )
...
@router.post("/browser/login", response_model=AuthSession)
def browser_login(
    request: LoginRequest,
    response: Response,
    db: Session = Depends(get_db),
) -> AuthSession:
    return _browser_session(response, login(request=request, db=db))


@router.post("/browser/qa-login", response_model=AuthSession)
def browser_qa_login(
    response: Response,
    db: Session = Depends(get_db),
) -> AuthSession:
    if not settings.qa_admin_enabled or settings.production_like:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="QA administrator access is unavailable.")

    email = (settings.qa_admin_email or "").strip().lower()
    user_record = db.scalar(select(UserRecord).where(UserRecord.email == email))
    if (
        user_record is None
        or not user_record.active
        or user_record.role != UserRole.admin.value
    ):
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="QA administrator access is not ready.",
        )
    ...
    market_id = next(
        (market_id for market_id in requested_market_ids if market_id in user_record.market_ids),
        user_record.default_market_id,
    )
    market_record = db.get(MarketRecord, market_id)
    if market_record is None or not market_record.active:
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="QA administrator market access is not ready.",
        )

    session = _auth_session_for_user(
        db,
        user_record=user_record,
        market_record=market_record,
        auth_method="qa_access",
        explicit_market_selection=False,
    )
    return _browser_session(response, session)
...
def complete_oidc_login(
    ...
    mapped_role, mapped_market_ids = identity_service.mapped_access_for_groups(userinfo.groups)
    group_role_required = bool(settings.oidc_group_role_map)
    group_market_required = bool(settings.oidc_group_market_map)
    if (group_role_required and mapped_role is None) or (
        group_market_required and state_record.market_id not in mapped_market_ids
    ):
        write_audit_event(
            db,
            actor=userinfo.email,
            ...
            details={
                "provider": _oidc_provider_key(oidc_config),
                "reason": "group_access_not_approved",
                "matched_role": mapped_role.value if mapped_role else None,
                "requested_market_id": state_record.market_id,
            },
            commit=True,
        )
        raise HTTPException(status.HTTP_403_FORBIDDEN, detail="SSO group access is not approved")
    ...
    if mapped_role is not None and user_record.role != mapped_role.value:
        previous_role = user_record.role
        user_record.role = mapped_role.value
        write_audit_event(
            db,
            actor="oidc",
            action="user.oidc.role_mapped",
            entity_type="user",
            entity_id=user_record.id,
            market_id=state_record.market_id,
            details={
...
@router.post("/browser/oidc/callback", response_model=AuthSession)
def complete_browser_oidc_login(
    request: OidcCallbackRequest,
    response: Response,
    db: Session = Depends(get_db),
) -> AuthSession:
    return _browser_session(response, complete_oidc_login(request=request, db=db))


@router.get("/browser/session", response_model=AuthSession)
def browser_session(
    ...
    return AuthSession(
        ...
        available_markets=_assigned_markets(db, context.user.market_ids),
    )
...
def browser_logout(
    ...
    if session_cookie:
        session_record = db.get(SessionRecord, session_cookie)
        if session_record is not None and session_record.revoked_at is None:
            ...
            write_audit_event(
                ...
                market_id=None,
                details={"auth_method": "browser_cookie"},
            )
            db.commit()
