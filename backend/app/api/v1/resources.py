...
from app.models.ticket_workspace import TicketWorkspace, TicketWorkspaceSuggestions
...
def _customer_list_query(
    ...
    *,
    ...
    query = select(CustomerRecord).where(
        CustomerRecord.market_id == market_id,
        CustomerRecord.active.is_(True),
    )
    if sentiment:
    ...
    if tag:
        query = query.where(func.lower(cast(CustomerRecord.tags, String)).like(f"%{tag.lower()}%"))
    if timezone_name:
        query = query.where(CustomerRecord.timezone == timezone_name)
    if created_since:
        query = query.where(CustomerRecord.created_at >= created_since)
    ...
    if search:
        ...
        query = query.where(
            or_(
                ...
                func.lower(CustomerRecord.job_title).like(term),
                func.lower(cast(CustomerRecord.contact_points, String)).like(term),
                func.lower(cast(CustomerRecord.tags, String)).like(term),
            )
        )
    return query
...
def list_customers(
    ...
    timezone_name: str | None = Query(None, alias="timezone", max_length=80),
    created_since: datetime | None = None,
...
def export_customers(
    ...
    query = _customer_list_query(
        ...
        timezone_name=timezone_name,
        created_since=created_since,
    ).order_by(CustomerRecord.name.asc(), CustomerRecord.id.asc())
    companies = {
    ...
    for record in db.scalars(query).all():
        contact_points = {point.get("channel", ""): point.get("value", "") for point in record.contact_points}
        writer.writerow(
            [
                record.name,
                record.job_title,
                companies.get(record.company_id or "", ""),
                record.email,
                contact_points.get("voice", contact_points.get("phone", "")),
                contact_points.get("facebook", ""),
                contact_points.get("twitter", ""),
                record.timezone,
                record.language,
                ", ".join(record.tags),
    ...
    filename = f"omni-contacts-{datetime.now(timezone.utc).date().isoformat()}.csv"
    return Response(
        output.getvalue(),
...
async def import_customers_csv(
    ...
    if not raw:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, detail="CSV file is empty")
    try:
    ...
    if not reader.fieldnames:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_CONTENT, detail="CSV header is missing")

    existing_emails = {
        value.lower()
        for value in db.scalars(
            select(CustomerRecord.email).where(CustomerRecord.market_id == context.market_id)
        ).all()
    }
    created: list[CustomerRecord] = []
    ...
    for index, source_row in enumerate(reader, start=2):
        ...
        name = row.get("contact") or row.get("name") or ""
        email = row.get("email address") or row.get("email") or ""
        if not name or "@" not in email:
            skipped.append({"row": str(index), "reason": "Contact and valid email are required"})
            continue
        if email.lower() in existing_emails:
            skipped.append({"row": str(index), "reason": "Email already exists"})
            continue
        contact_points: list[dict[str, Any]] = []
        phone = row.get("work phone") or row.get("phone") or ""
        if phone:
            contact_points.append({"channel": "voice", "value": phone, "verified": False})
        facebook = row.get("facebook") or ""
        if facebook:
            contact_points.append(
                {"channel": "facebook", "value": facebook, "verified": False}
            )
        ...
        record = CustomerRecord(
            ...
            timezone=row.get("time zone") or row.get("timezone") or "Africa/Lagos",
    ...
    for record in created:
        db.refresh(record)
        _sync_customer_to_store(state, customer_from_record(record))
    return {
        "created": len(created),
        "skipped": len(skipped),
        "errors": skipped[:100],
...
def read_customer(
    ...
    return {"customer": customer, "company": company, "tickets": ticket_contexts}
...
@router.post("/customers/{source_customer_id}/merge/{target_customer_id}")
def merge_customers(
    ...
    if source_customer_id == target_customer_id:
        raise HTTPException(status.HTTP_409_CONFLICT, detail="Choose a different contact to merge")
    source = _customer_record_or_404(db, source_customer_id, context.market_id)
    target = _customer_record_or_404(db, target_customer_id, context.market_id)
    if not source.active or not target.active:
        raise HTTPException(status.HTTP_409_CONFLICT, detail="Only active contacts can be merged")

    target_open_case = db.scalar(
        select(CaseRecord).where(
            CaseRecord.market_id == context.market_id,
            CaseRecord.customer_id == target_customer_id,
    ...
    source_cases = db.scalars(
        select(CaseRecord).where(
            CaseRecord.market_id == context.market_id,
            CaseRecord.customer_id == source_customer_id,
        )
    ).all()
    for source_case in source_cases:
        if source_case.status == "open" and target_open_case is not None:
            db.execute(
                update(TicketRecord)
                .where(TicketRecord.case_id == source_case.id)
                .values(case_id=target_open_case.id)
            )
            db.execute(
                update(ChatConversationRecord)
                .where(ChatConversationRecord.case_id == source_case.id)
                .values(case_id=target_open_case.id)
            )
            source_case.status = "closed"
            source_case.summary = (
                f"{source_case.summary}\nMerged into {target_open_case.public_id}."
            ).strip()
            db.flush()
        source_case.customer_id = target_customer_id
        if source_case.status == "open":
            target_open_case = source_case

    db.execute(
        update(TicketRecord)
        .where(
            TicketRecord.market_id == context.market_id,
            TicketRecord.customer_id == source_customer_id,
        )
    ...
    db.execute(
        update(ChatConversationRecord)
        .where(
            ChatConversationRecord.market_id == context.market_id,
            ChatConversationRecord.customer_id == source_customer_id,
        )
        .values(customer_id=target_customer_id)
    )
    db.execute(
        update(CsatFeedbackRecord)
        .where(
            CsatFeedbackRecord.market_id == context.market_id,
            CsatFeedbackRecord.customer_id == source_customer_id,
        )
        .values(customer_id=target_customer_id)
    )
    db.execute(
        ...
        .where(
            ServiceAppointmentRecord.market_id == context.market_id,
            ServiceAppointmentRecord.customer_id == source_customer_id,
        )
        .values(customer_id=target_customer_id)
    )
    db.execute(
        ...
        .where(
            CustomerIdentityRecord.market_id == context.market_id,
            CustomerIdentityRecord.customer_id == source_customer_id,
        )
        .values(customer_id=target_customer_id)
    )

    source_participants = db.scalars(
        select(ChatParticipantRecord).where(
            ChatParticipantRecord.market_id == context.market_id,
            ChatParticipantRecord.customer_id == source_customer_id,
        )
    ).all()
    for participant in source_participants:
        participant_duplicate = db.scalar(
            select(ChatParticipantRecord).where(
                ChatParticipantRecord.conversation_id == participant.conversation_id,
                ChatParticipantRecord.participant_type == participant.participant_type,
                ChatParticipantRecord.customer_id == target_customer_id,
            )
        )
        if participant_duplicate:
            db.delete(participant)
        else:
            participant.customer_id = target_customer_id

    source_consents = db.scalars(
        select(CampaignConsentRecord).where(
            CampaignConsentRecord.market_id == context.market_id,
            CampaignConsentRecord.customer_id == source_customer_id,
        )
    ).all()
    for consent in source_consents:
        target_consent = db.scalar(
            select(CampaignConsentRecord).where(
                CampaignConsentRecord.market_id == context.market_id,
                CampaignConsentRecord.customer_id == target_customer_id,
                CampaignConsentRecord.channel == consent.channel,
            )
        )
        if target_consent:
            db.execute(
                update(CampaignDeliveryRecord)
                .where(CampaignDeliveryRecord.consent_id == consent.id)
                .values(consent_id=target_consent.id)
            )
            db.delete(consent)
        else:
            consent.customer_id = target_customer_id

    source_deliveries = db.scalars(
        select(CampaignDeliveryRecord).where(
            CampaignDeliveryRecord.market_id == context.market_id,
            CampaignDeliveryRecord.customer_id == source_customer_id,
        )
    ).all()
    for delivery in source_deliveries:
        delivery_duplicate = db.scalar(
            select(CampaignDeliveryRecord).where(
                CampaignDeliveryRecord.campaign_id == delivery.campaign_id,
                CampaignDeliveryRecord.customer_id == target_customer_id,
            )
        )
        if delivery_duplicate:
            merged_history = list(
                delivery_duplicate.provider_payload.get("merged_deliveries", [])
            )
            merged_history.append(
                {"id": delivery.id, "status": delivery.status, "recipient": delivery.recipient}
            )
            delivery_duplicate.provider_payload = {
                **delivery_duplicate.provider_payload,
                "merged_deliveries": merged_history,
            }
            db.delete(delivery)
        else:
            delivery.customer_id = target_customer_id

    target.contact_points = list(
        {
            (point.get("channel", ""), point.get("value", "")): point
            for point in [*(target.contact_points or []), *(source.contact_points or [])]
        }.values()
    )
    target.tags = list(dict.fromkeys([*(target.tags or []), *(source.tags or [])]))
    if source.notes:
        target.notes = f"{target.notes}\nMerged from {source.name}: {source.notes}".strip()
    source.active = False
    source.merged_into_id = target_customer_id
    write_audit_event(
        db,
        actor=context.user.email,
        ...
        market_id=context.market_id,
        details={"source_customer_id": source_customer_id},
    )
    db.commit()
    db.refresh(target)
    db.refresh(source)
    ...
    for ticket in state.tickets.values():
        if ticket.customer_id == source_customer_id:
            ticket.customer_id = target_customer_id
    _sync_customer_to_store(state, customer_from_record(target))
    _sync_customer_to_store(state, customer_from_record(source))
    return {
        "customer": customer_from_record(target),
        "merged_customer_id": source_customer_id,
    }


@router.get("/ticket-views", response_model=list[TicketView])
def list_ticket_views(
    context: RequestContext = Depends(require_context),
    db: Session = Depends(get_db),
) -> list[TicketView]:
    return ticket_repository.list_views(
        db,
        market_id=context.market_id,
...
@router.post("/ticket-views", response_model=TicketView, status_code=status.HTTP_201_CREATED)
def create_ticket_view(
    payload: CreateTicketViewRequest,
    context: RequestContext = Depends(require_context),
    state: InMemoryStore = Depends(get_store),
    db: Session = Depends(get_db),
) -> TicketView:
    return ticket_repository.create_view(
        db,
        state,
        market_id=context.market_id,
        user_id=context.user.id,
        is_admin=context.user.role.value == "admin",
        payload=payload,
    )


@router.patch("/ticket-views/{view_id}", response_model=TicketView)
def update_ticket_view(
    view_id: str,
    payload: UpdateTicketViewRequest,
    context: RequestContext = Depends(require_context),
    state: InMemoryStore = Depends(get_store),
    db: Session = Depends(get_db),
) -> TicketView:
    return ticket_repository.update_view(
        db,
        state,
        market_id=context.market_id,
        user_id=context.user.id,
...
def list_tickets(
    ...
    team: str | None = None,
    created_period: str | None = Query(None, pattern="^(today|7d|30d)$"),
    tag: str | None = Query(None, max_length=180),
    ...
    if view_id:
        view = ticket_repository.get_view(
            db,
            market_id=context.market_id,
            user_id=context.user.id,
            view_id=view_id,
        ...
        sort_by = sort_by or view.sort_by
        sort_order = sort_order or view.sort_order
    status_filter = status_filter or view_filters.get("status")
    customer_id = customer_id or view_filters.get("customer_id")
    assignee_id = assignee_id or view_filters.get("assignee_id")
    team = team or view_filters.get("team")
    created_period = created_period or view_filters.get("created_period")
    tag = tag or view_filters.get("tag")
    try:
        channel = channel or (ChannelType(view_filters["channel"]) if view_filters.get("channel") else None)
        priority = priority or (Priority(view_filters["priority"]) if view_filters.get("priority") else None)
    except ValueError as error:
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Saved ticket view has invalid filters") from error
    effective_sort_by = sort_by or "updated_at"
...
@router.get("/tickets/{ticket_id}/workspace", response_model=TicketWorkspace)
def read_ticket_workspace(
    ticket_id: str,
    response: Response,
    context: RequestContext = Depends(require_context),
    state: InMemoryStore = Depends(get_store),
    ...
    ticket: Ticket = ticket_context["ticket"]
    _set_resource_etag(response, ticket)

    watcher_records = list(
        db.scalars(
            select(TicketWatcherRecord).where(
                TicketWatcherRecord.market_id == context.market_id,
                TicketWatcherRecord.ticket_id == ticket_id,
            )
        ).all()
    )
    watcher_users = [db.get(UserRecord, watcher.user_id) for watcher in watcher_records]
    time_entries = personal_workspace_repository.list_time_entries(
        db,
        market_id=context.market_id,
        ticket_id=ticket_id,
    )

    linked_records: list[TicketRecord] = []
    if ticket.case_id:
        linked_records = list(
            db.scalars(
                select(TicketRecord)
                .where(
                    TicketRecord.market_id == context.market_id,
                    TicketRecord.case_id == ticket.case_id,
    ...
    for handoff in ticket_context["handoffs"]:
        if handoff.linked_ticket_id and handoff.linked_ticket_id not in linked_ids:
            linked = db.get(TicketRecord, handoff.linked_ticket_id)
            if linked is not None and linked.market_id == context.market_id:
                linked_records.append(linked)
                linked_ids.add(linked.id)

    ordered_ticket_ids = list(
        db.scalars(
            select(TicketRecord.id)
            .where(TicketRecord.market_id == context.market_id)
            .order_by(TicketRecord.updated_at.desc(), TicketRecord.id.desc())
        ).all()
    )
    current_index = ordered_ticket_ids.index(ticket_id)
    previous_ticket_id = ordered_ticket_ids[current_index - 1] if current_index > 0 else None
    next_ticket_id = (
        ordered_ticket_ids[current_index + 1]
        if current_index + 1 < len(ordered_ticket_ids)
        else None
    )

    service_tasks = [
        appointment
        for appointment in management_repository.list_service_appointments(
            db, state, context.market_id
        )
        if appointment.customer_id == ticket.customer_id
    ]
    case = (
        case_repository.get_case(db, context.market_id, ticket.case_id)
        if ticket.case_id
        else None
    )
    allowed_actions = ["view_activity", "next", "previous"]
    if Permission.operations_write in context.user.effective_permissions:
        ...
        allowed_actions.append(
            "reopen" if ticket.status.value in {"solved", "closed"} else "close"
        )

    return TicketWorkspace(
        ticket=ticket,
        case=case,
        customer=ticket_context["customer"],
        company=ticket_context["company"],
        assignee=ticket_context["assignee"],
        timeline=ticket_context["timeline"],
        properties={
            ...
            "sla": ticket.sla.model_dump(mode="json"),
        },
        watchers=[user_from_record(user) for user in watcher_users if user is not None],
        watching=any(watcher.user_id == context.user.id for watcher in watcher_records),
        tasks=ticket.tasks,
        time_entries=time_entries,
        attachments=ticket_context["attachments"],
        linked_tickets=[ticket_from_record(record) for record in linked_records],
        service_tasks=service_tasks,
        handoffs=ticket_context["handoffs"],
        suggestions=TicketWorkspaceSuggestions(
            knowledge=ticket_context["knowledge_suggestions"],
            macros=ticket_context["macro_suggestions"],
            duplicates=ticket_context["duplicate_suggestions"],
        ),
...
def update_ticket(
    ...
    if request.expected_version is not None and request.expected_version != record.version:
        raise HTTPException(
            status.HTTP_412_PRECONDITION_FAILED,
            detail={
                "message": "Ticket has changed; refresh before retrying.",
...
@router.post("/tickets/{ticket_id}/forward", response_model=TimelineEvent)
def forward_ticket(
    ticket_id: str,
    ...
    event = ticket_repository.forward(
        db,
        state,
        ticket_id,
        request,
    ...
    _set_resource_etag(response, _ticket_record_or_404(db, ticket_id, context.market_id))
    return event


@router.post("/tickets/{ticket_id}/tasks", response_model=Ticket, status_code=201)
def create_ticket_task(
    ticket_id: str,
    request: CreateTicketTaskRequest,
    response: Response,
    if_match: str | None = Header(None, alias="If-Match"),
    context: RequestContext = Depends(require_context),
    state: InMemoryStore = Depends(get_store),
    ...
    ticket = ticket_repository.create_task(
        db,
        state,
        ticket_id,
        request,
...
async def ingest_signed_webhook(
    ...
    if webhook_event is not None and webhook_event.kind == "delivery_receipt":
        ...
        campaign_delivery = campaign_service.record_delivery_receipt(
            db,
            market_id=market_record.id,
            provider=provider_type,
            ...
            campaign_delivery_id=(
                ...
                if (receipt.outbound_message_id or "").startswith("campaign_delivery_")
                else None
            ),
        ...
        if campaign_delivery is not None:
            response["campaign_delivery"] = campaign_delivery
        else:
            response["outbound_message"] = outbound_message
        return response
