    ...
    *,
    ...
    return db.scalar(
        select(CaseRecord).where(
            ...
            CaseRecord.status == CaseStatus.open.value,
        )
    )


def _ensure_active_case(
    db: Session,
    *,
    market_id: str,
    customer_id: str,
    ...
    # Locking the customer serializes competing intake requests in PostgreSQL.
    db.execute(
    ...
    existing = _active_case_for_customer(
        db,
        market_id=market_id,
...
def _ticket_list_query(
    *,
    ...
    if status_filter:
        ...
        query = query.where(TicketRecord.status.in_(statuses))
    if channel:
    ...
    period = (created_period or "").strip().lower()
    if period:
        now = utc_now()
        if period == "today":
            ...
            raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Invalid created period")
        query = query.where(TicketRecord.created_at >= start)
    for tag_value in [value.strip().lower() for value in (tag or "").split(",") if value.strip()]:
        query = query.where(func.lower(cast(TicketRecord.tags, String)).like(f'%"{tag_value}"%'))
    ...
    if search:
        ...
        query = query.where(
            or_(
                ...
                func.lower(cast(TicketRecord.tags, String)).like(term),
            )
        )
    return query


def _ticket_view_from_record(record: TicketViewRecord) -> TicketView:
...
def _append_threaded_connector_reply(
    ...
    *,
    ...
    if previous_status != TicketStatus.open.value:
        _apply_status_with_stamps(target_ticket, TicketStatus.open.value, utc_now())
...
def _apply_status_with_stamps(record: TicketRecord, new_status: str, now: datetime) -> None:
    ...
    solved<->closed moves keep the original resolved_at and only adjust closed_at."""
    ...
    elif previous in _RESOLVED_STATUSES:
...
def _sync_handoff_for_resolved_child(
    db: Session,
    state: InMemoryStore,
    record: TicketRecord,
    market_id: str,
    ...
    if (
        handoff_record is None
        or handoff_record.market_id != market_id
        or handoff_record.status in {HandoffStatus.resolved.value, HandoffStatus.cancelled.value}
    ):
        return
    ...
    if parent is not None and parent.market_id == market_id:
        _add_timeline_record(
            ...
            body=f"Handoff to {handoff_record.to_team} completed — {record.public_id} resolved.",
            public=False,
...
def _sync_ticket(db: Session, state: InMemoryStore, ticket: TicketRecord) -> Ticket:
    if not _sla_is_frozen(ticket):
        current_ticket = ticket_from_record(ticket)
        refreshed_sla = sla_service.refresh(current_ticket.sla)
        serialized_sla = refreshed_sla.model_dump(mode="json")
        if ticket.sla != serialized_sla:
            ticket.sla = serialized_sla
            # SQLAlchemy's version column is incremented during flush. Serialize only
            # after that flush so clients never receive a stale optimistic-lock value.
            db.flush()
...
class TicketRepository:
    def list_views(
        ...
        *,
        ...
        records = db.scalars(
            select(TicketViewRecord)
            .where(
                TicketViewRecord.market_id == market_id,
                TicketViewRecord.active.is_(True),
                or_(
                    TicketViewRecord.owner_user_id.is_(None),
                    TicketViewRecord.owner_user_id == user_id,
                ),
            )
            .order_by(TicketViewRecord.position.asc(), TicketViewRecord.name.asc())
        ).all()
        return [_ticket_view_from_record(record) for record in records]
    ...
    def get_view(
        ...
        *,
        ...
        record = db.get(TicketViewRecord, view_id)
        if (
            record is None
            or record.market_id != market_id
            or not record.active
            or record.owner_user_id not in {None, user_id}
        ):
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Ticket view not found")
        return _ticket_view_from_record(record)
    ...
    def create_view(
        ...
        *,
        ...
        existing = db.scalar(
            select(TicketViewRecord).where(
                TicketViewRecord.market_id == market_id,
                TicketViewRecord.owner_user_id == owner_user_id,
                func.lower(TicketViewRecord.name) == payload.name.strip().lower(),
            )
        )
        if existing is not None:
            raise HTTPException(status.HTTP_409_CONFLICT, detail="A ticket view with this name already exists")
        record = TicketViewRecord(
            id=_new_id("ticket_view"),
            market_id=market_id,
        ...
        _audit(
            ...
            state,
            actor=user_id,
            action="ticket_view.create",
            entity_type="ticket_view",
            entity_id=record.id,
            market_id=market_id,
            details={"name": record.name, "shared": payload.shared, "filters": record.filters},
        )
        db.commit()
        db.refresh(record)
        return _ticket_view_from_record(record)
    ...
    def update_view(
        ...
        *,
        ...
        if record is None or record.market_id != market_id:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Ticket view not found")
        if record.owner_user_id != user_id and not is_admin:
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="Admin access required")
    ...
    def create_ticket(
        ...
        *,
        ...
        active_case, case_created = _ensure_active_case(
            db,
            market_id=market_id,
            customer_id=customer.id,
        ...
        ticket_record.case_id = active_case.id
        ...
        if case_created:
            _audit(
                ...
                state,
                actor=actor,
                action="case.create_from_ticket",
                entity_type="case",
                entity_id=active_case.id,
                market_id=market_id,
                details={"ticket_id": ticket.id, "customer_id": customer.id},
            )
        if notify_customer:
    ...
    def update_ticket(
        ...
        # Close-out side effects fire only when ENTERING the resolved set — a
        # solved -> closed move must not re-post the note or re-email the customer.
        resolved_now = (
            status_changed
            and record.status in _RESOLVED_STATUSES
            and previous_status not in _RESOLVED_STATUSES
        )
        if resolved_now:
            _sync_handoff_for_resolved_child(db, state, record, market_id, now)
        note_text = (resolution_note or "").strip()
        # A deliberately PAUSED "Ticket resolved" notification suppresses the customer
        # email even when the agent leaves notify on; a missing config row falls back
        # to sensible defaults so fresh installs still notify.
        resolution_notification = db.scalar(
            select(EmailNotificationRecord).where(
                EmailNotificationRecord.market_id == market_id,
                EmailNotificationRecord.event == "ticket_resolved",
            )
        )
        notification_paused = resolution_notification is not None and not resolution_notification.active
        if notification_paused and notify_customer:
        ...
        if resolved_now and notify_customer:
            ...
            if to_email:
                notification = resolution_notification
    ...
    def run_scenario(
        ...
        *,
        ...
        now = utc_now()
        # Pre-validate the whole bundle before touching the ticket: action values are
        # free text in Setup, and committing an invalid status/priority would corrupt
        # the record and 500 every ticket read for the market. Common UI-vocabulary
        # synonyms are normalised instead of rejected.
        status_synonyms = {"resolved": TicketStatus.solved.value, "new": TicketStatus.open.value}
        priority_synonyms = {"medium": Priority.normal.value}
        ...
        for action in scenario.actions or []:
            ...
            if action_type == "set_status" and value:
                value = status_synonyms.get(value.lower(), value.lower())
                if value not in valid_statuses:
                    raise HTTPException(
                        status.HTTP_422_UNPROCESSABLE_CONTENT,
                        detail=f"Scenario '{scenario.name}': invalid status '{action.get('value')}'.",
                    )
            elif action_type == "set_priority" and value:
                value = priority_synonyms.get(value.lower(), value.lower())
                if value not in valid_priorities:
                    raise HTTPException(
                        status.HTTP_422_UNPROCESSABLE_CONTENT,
                        detail=f"Scenario '{scenario.name}': invalid priority '{action.get('value')}'.",
                    )
            normalized_actions.append((action_type, value))
        # The scenario engine honours the same close-out guard as manual transitions.
        closes_out = any(
            action_type == "set_status" and value in _RESOLVED_STATUSES
            for action_type, value in normalized_actions
        )
        if closes_out and record.status not in _RESOLVED_STATUSES:
            open_children = _open_handoff_children(db, record)
            if open_children:
                raise HTTPException(
                    status.HTTP_422_UNPROCESSABLE_CONTENT,
        ...
        if record.status in _RESOLVED_STATUSES and previous_status not in _RESOLVED_STATUSES:
            _sync_handoff_for_resolved_child(db, state, record, market_id, now)
        record.updated_at = now
        _add_timeline_record(
            db,
    ...
    def forward(
        ...
        *,
        ...
        event = _add_timeline_record(
            ...
            state,
            ticket_record,
            event_type=TimelineEventType.forwarded,
            channel=ChannelType.email,
            actor=actor,
            body=request.body,
            public=False,
        ...
        if timeline_record is None:
            raise HTTPException(
                ...
                detail="Timeline event missing",
            )
        idempotency_key = request.idempotency_key or f"{ticket_id}:{event.id}:forward:email"
        outbound_message = outbound_repository.queue_email(
            db,
            state,
            ticket=ticket_record,
            ...
            subject=request.subject,
            body=request.body,
            source="ticket_forward",
            idempotency_key=idempotency_key,
            audit_action="ticket.forward.queue",
        )
        outbound_repository.process_message(
            db,
            state,
    ...
    def create_task(
        ...
        *,
        ...
        ticket_record = _ticket_record_or_404(db, ticket_id, market_id)
        task = _task(request.label.strip())
        ticket_record.tasks = [*(ticket_record.tasks or []), task.model_dump(mode="json")]
        ticket_record.updated_at = utc_now()
        _add_timeline_record(
            db,
            state,
            ...
            channel=ChannelType.internal,
            actor=actor,
            body=f"Child task created: {task.label}",
            public=False,
        ...
        _audit(
            ...
            entity_type="ticket",
            entity_id=ticket_record.id,
            market_id=market_id,
            details={"task_id": task.id, "task_label": task.label},
        )
        db.commit()
        db.refresh(ticket_record)
        return _sync_ticket(db, state, ticket_record)
    ...
    def portal_customer_reply(
        ...
        *,
        ...
        return self.customer_channel_reply(
            db,
            state,
            ticket_id,
            market_id,
            ...
            source="portal",
            system_actor="customer-portal",
        )

    def customer_channel_reply(
        self,
        db: Session,
        state: InMemoryStore,
        ticket_id: str,
        ...
        *,
        ...
        event = _add_timeline_record(
            ...
            body=body,
            public=True,
            metadata={"source": source, "submitted_by": submitted_by},
        )
        if previous_status != TicketStatus.open.value:
            _apply_status_with_stamps(ticket_record, TicketStatus.open.value, utc_now())
            _add_timeline_record(
                db,
                state,
                ticket_record,
                ...
                actor=system_actor,
                body=(
                    f"Customer replied through {source.replace('_', ' ')}; "
                    f"ticket moved from {previous_status} to open."
                ),
                public=False,
                metadata={"previous_status": previous_status, "new_status": TicketStatus.open.value},
            )
        ticket_record.tags = sorted(set(ticket_record.tags or []) | {source, "customer-replied"})
        ticket_record.updated_at = utc_now()
        _audit(
            db,
            state,
            actor=system_actor,
            action=f"ticket.{source}_reply",
            entity_type="ticket",
            entity_id=ticket_id,
            market_id=market_id,
            details={
    ...
    def update_handoff(
        ...
        # Handoff cancelled -> the auto-created child closes too, so it can't strand
        # the parent behind the open-children guard.
        terminal_transitions = {
            HandoffStatus.resolved.value: (TicketStatus.solved.value, "completed"),
            HandoffStatus.cancelled.value: (TicketStatus.closed.value, "cancelled"),
        }
        transition = (
            terminal_transitions.get(handoff_record.status)
            if handoff_record.status != status_was
            else None
        )
        if transition and handoff_record.linked_ticket_id:
            child_status, verb = transition
            ...
            if child is not None and child.market_id == market_id and child.status not in _RESOLVED_STATUSES:
                sync_now = utc_now()
                _apply_status_with_stamps(child, child_status, sync_now)
                ...
                _add_timeline_record(
                    ...
                    channel=ChannelType.internal,
                    actor="handoff-service",
                    body=f"Ticket {'resolved' if child_status == TicketStatus.solved.value else 'closed'} — handoff to {handoff_record.to_team} {verb}.",
                    public=False,
...
class CaseRepository:
    ...
    def create_case(
        ...
        *,
        ...
        record, created = _ensure_active_case(
            db,
            market_id=market_id,
            customer_id=customer.id,
            title=payload.title,
            ...
            actor=payload.opened_by or actor,
        )
        ...
        record.updated_at = utc_now()
        db.flush()
        action = "case.create" if created else "case.reuse_active"
        self._audit_case(db, state, record, actor, action, {"ticket_ids": [t.id for t in attached]})
        db.commit()
        return self.get_case(db, market_id, record.id)
    ...
    def update_case(
        ...
        *,
        ...
        if payload.status is not None:
            if payload.status != CaseStatus.open:
                active_children = db.scalars(
                    select(TicketRecord).where(
                        TicketRecord.market_id == market_id,
                        TicketRecord.case_id == record.id,
                        TicketRecord.status.not_in(_RESOLVED_STATUSES),
                    )
                ).all()
                if active_children:
                    raise HTTPException(
                        status.HTTP_422_UNPROCESSABLE_CONTENT,
                        detail=(
                            "Resolve or close active child tickets first: "
                            + ", ".join(ticket.public_id for ticket in active_children)
                        ),
                    )
            elif record.status != CaseStatus.open.value:
                existing = _active_case_for_customer(
                    db,
                    market_id=market_id,
                ...
                if existing is not None and existing.id != record.id:
                    raise HTTPException(
                        status.HTTP_409_CONFLICT,
                        detail=f"Customer already has active case {existing.public_id}",
                    )
