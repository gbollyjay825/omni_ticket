...
class PersonalTaskRecord(TimestampMixin, Base):
...
class TicketWatcherRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("ticket_id", "user_id", name="uq_ticket_watcher_ticket_user"),
    )


class TicketViewRecord(TimestampMixin, Base):
    ...
    sort_by: Mapped[str] = mapped_column(String(40), default="updated_at")
    sort_order: Mapped[str] = mapped_column(String(8), default="desc")
    position: Mapped[int] = mapped_column(Integer, default=100)
    active: Mapped[bool] = mapped_column(Boolean, default=True, index=True)

    __table_args__ = (
        UniqueConstraint("market_id", "owner_user_id", "name", name="uq_ticket_view_owner_name"),
    )


class TicketTimeEntryRecord(TimestampMixin, Base):
...
class ReportDeliveryRecord(TimestampMixin, Base):
    __tablename__ = "report_deliveries"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    market_id: Mapped[str] = mapped_column(ForeignKey("markets.id"), index=True)
    saved_report_id: Mapped[str] = mapped_column(ForeignKey("saved_reports.id"), index=True)
    period_key: Mapped[str] = mapped_column(String(32), index=True)
    report_type: Mapped[str] = mapped_column(String(40), index=True)
    cadence: Mapped[str] = mapped_column(String(20), index=True)
...
class CampaignRecord(TimestampMixin, Base):
    ...
    template_name: Mapped[str] = mapped_column(String(180), default="")
    template_language: Mapped[str] = mapped_column(String(20), default="en_US")
    template_variables: Mapped[list[str]] = mapped_column(JSON, default=list)
    ...
    __table_args__ = (
        UniqueConstraint("market_id", "name", name="uq_campaign_market_name"),
    )


class CampaignConsentRecord(TimestampMixin, Base):
    __tablename__ = "campaign_consents"
    ...
    evidence: Mapped[str] = mapped_column(String(1000), default="")
    captured_by: Mapped[str] = mapped_column(String(180), nullable=False)
...
class CampaignDeliveryRecord(TimestampMixin, Base):
    ...
    market_id: Mapped[str] = mapped_column(ForeignKey("markets.id"), index=True)
    customer_id: Mapped[str] = mapped_column(ForeignKey("customers.id"), index=True)
    consent_id: Mapped[str] = mapped_column(ForeignKey("campaign_consents.id"), index=True)
    channel: Mapped[str] = mapped_column(String(32), index=True)
    ...
    __table_args__ = (
        UniqueConstraint(
            "campaign_id",
            "customer_id",
            name="uq_campaign_delivery_campaign_customer",
        ),
        UniqueConstraint(
            "market_id",
            "channel",
...
class SegmentRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("market_id", "name", name="uq_segment_market_name"),
    )


class AiAgentRecord(TimestampMixin, Base):
    ...
    channels: Mapped[list[str]] = mapped_column(JSON, default=list)
    languages: Mapped[list[str]] = mapped_column(JSON, default=list)
    handoff_team: Mapped[str] = mapped_column(String(160), default="")
    ...
    __table_args__ = (
        UniqueConstraint("market_id", "name", name="uq_ai_agent_market_name"),
    )
...
class CustomerRecord(TimestampMixin, Base):
    ...
    notes: Mapped[str] = mapped_column(Text, default="")
    job_title: Mapped[str] = mapped_column(String(180), default="")
...
class ConversationTopicRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("market_id", "name", name="uq_conversation_topic_market_name"),
    )


class ChatConversationRecord(TimestampMixin, Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="open", index=True)
    priority: Mapped[str] = mapped_column(String(32), default="normal", index=True)
    topic_id: Mapped[str | None] = mapped_column(ForeignKey("conversation_topics.id"), index=True)
    assignee_id: Mapped[str | None] = mapped_column(ForeignKey("users.id"), index=True)
    assigned_group_id: Mapped[str | None] = mapped_column(
        ForeignKey("support_groups.id"), index=True
    )
    source_account_id: Mapped[str | None] = mapped_column(String(160), index=True)
    external_id: Mapped[str | None] = mapped_column(String(255), index=True)
    version: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    ...
    conversation_metadata: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ChatMessageRecord(TimestampMixin, Base):
    ...
    delivery_state: Mapped[str] = mapped_column(
        String(32), default="pending_provider", index=True
    )
    ...
    reply_to_id: Mapped[str | None] = mapped_column(ForeignKey("chat_messages.id"))
    version: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
...
class ChatParticipantRecord(TimestampMixin, Base):
    ...
    participant_type: Mapped[str] = mapped_column(String(24), index=True)
...
class ConversationAssignmentRecord(Base):
    ...
    from_user_id: Mapped[str | None] = mapped_column(String(64))
    to_user_id: Mapped[str | None] = mapped_column(String(64), index=True)
    from_group_id: Mapped[str | None] = mapped_column(String(64))
    to_group_id: Mapped[str | None] = mapped_column(String(64), index=True)
    reason: Mapped[str] = mapped_column(String(500), default="")
    routed_by: Mapped[str] = mapped_column(String(180), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, index=True)


class ConversationViewRecord(TimestampMixin, Base):
    ...
    name: Mapped[str] = mapped_column(String(160), nullable=False)
    filters: Mapped[dict] = mapped_column(JSON, default=dict)
    sort_by: Mapped[str] = mapped_column(String(40), default="last_message_at")
    sort_order: Mapped[str] = mapped_column(String(8), default="desc")
    position: Mapped[int] = mapped_column(Integer, default=100)
    active: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
...
class MessageReceiptRecord(Base):
...
class ConversationTicketLinkRecord(Base):
    __tablename__ = "conversation_ticket_links"
    ...
    ticket_id: Mapped[str] = mapped_column(ForeignKey("tickets.id"), index=True)
    relationship: Mapped[str] = mapped_column(String(32), default="linked")
    created_by: Mapped[str] = mapped_column(String(180), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
...
class ConversationAttachmentRecord(TimestampMixin, Base):
    ...
    scan_result: Mapped[str] = mapped_column(Text, default="")
...
class FeatureFlagRecord(TimestampMixin, Base):
    ...
    updated_by: Mapped[str] = mapped_column(String(180), nullable=False)

    __table_args__ = (
        UniqueConstraint("market_id", "key", name="uq_feature_flag_market_key"),
    )


class RealtimeEventRecord(Base):
    """Durable event cursor used for client catch-up and Redis fan-out."""

    __tablename__ = "realtime_events"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    organization_id: Mapped[str] = mapped_column(String(64), default="wakanow", index=True)
    market_id: Mapped[str] = mapped_column(ForeignKey("markets.id"), index=True)
...
class CustomerIdentityRecord(TimestampMixin, Base):
    """Normalized customer identity used for cross-channel resolution and audited imports."""
    ...
    market_id: Mapped[str] = mapped_column(ForeignKey("markets.id"), index=True)
    customer_id: Mapped[str] = mapped_column(ForeignKey("customers.id"), index=True)
    provider: Mapped[str] = mapped_column(String(40), default="omni", index=True)
    ...
    display_value: Mapped[str] = mapped_column(String(500), default="")
...
class ImportRunRecord(TimestampMixin, Base):
    ...
    started_by: Mapped[str] = mapped_column(String(180), default="migration-cli")
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
    finished_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    source_cursor: Mapped[dict] = mapped_column(JSON, default=dict)
    next_cursor: Mapped[dict] = mapped_column(JSON, default=dict)
    statistics: Mapped[dict] = mapped_column(JSON, default=dict)
    error_summary: Mapped[str] = mapped_column(Text, default="")


class ImportCursorRecord(TimestampMixin, Base):
    ...
    checkpoint_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
...
class ExternalIdMappingRecord(TimestampMixin, Base):
    ...
    internal_id: Mapped[str] = mapped_column(String(64), index=True)
    source_updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    payload_hash: Mapped[str] = mapped_column(String(64), default="")
    mapping_metadata: Mapped[dict] = mapped_column(JSON, default=dict)
...
class ImportReconciliationRecord(TimestampMixin, Base):
    ...
    source_checksum: Mapped[str] = mapped_column(String(128), default="")
    sample_size: Mapped[int] = mapped_column(Integer, default=0)
    sample_failures: Mapped[int] = mapped_column(Integer, default=0)
    missing_attachments: Mapped[int] = mapped_column(Integer, default=0)
    source_snapshot_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True)
    recorded_by: Mapped[str] = mapped_column(String(180), nullable=False)
    notes: Mapped[str] = mapped_column(Text, default="")


class ImportCutoverDecisionRecord(TimestampMixin, Base):
    __tablename__ = "import_cutover_decisions"
    ...
    rollback_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class OutboundMessageRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_outbound_message_single_source",
        ),
    )
...
class AiDecisionRecord(TimestampMixin, Base):
    ...
    input_reference: Mapped[str] = mapped_column(String(180), default="rules-engine")
