...
def _mark_chat_delivery_state(
    ...
    *,
    ...
    if not message.chat_message_id:
        return
    chat_message = db.get(ChatMessageRecord, message.chat_message_id)
    if chat_message is None:
        return
    chat_message.delivery_state = delivery_state
    if provider_message_id:
        chat_message.provider_message_id = provider_message_id
    chat_message.message_metadata = {
        **(chat_message.message_metadata or {}),
        "outbound_message_id": message.id,
        "connector_event_id": message.connector_event_id,
        "delivery_status": delivery_state,
        "delivery_error": error,
    }
    chat_message.updated_at = utc_now()
...
class OutboundRepository:
    ...
    def queue_conversation_reply(
        ...
        *,
        ...
        resolved_key = idempotency_key or (
            f"{conversation.id}:{chat_message.id}:{provider.value}"
        )
        existing = db.scalar(
            select(OutboundMessageRecord).where(
                OutboundMessageRecord.market_id == conversation.market_id,
                OutboundMessageRecord.provider == provider.value,
                OutboundMessageRecord.idempotency_key == resolved_key,
            )
        )
        ...
        message_record = OutboundMessageRecord(
            ...
            payload={
                ...
                "chat_message_id": chat_message.id,
                "attachment_ids": list(
                    (chat_message.content or {}).get("attachment_ids", [])
                ),
            },
        )
        db.add(message_record)
        ...
        _mark_chat_delivery_state(
            db,
            message=message_record,
            delivery_state=OutboundMessageStatus.queued.value,
        )
        _audit(
            db,
            state,
        ...
        domain_message = outbound_message_from_record(message_record)
    ...
    def process_message(
        ...
        *,
        ...
        ticket = db.get(TicketRecord, message.ticket_id) if message.ticket_id else None
        conversation = (
            db.get(ChatConversationRecord, message.conversation_id)
            if message.conversation_id
            else None
        )
        if ticket is not None and ticket.market_id != market_id:
            ticket = None
        if conversation is not None and conversation.market_id != market_id:
            conversation = None
        if ticket is None and conversation is None:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Outbound source not found")
        customer_id = ticket.customer_id if ticket is not None else conversation.customer_id
        ...
        if ready:
            ...
            if ticket is not None:
                ...
                _mark_chat_delivery_state(
                    db,
                    message=message,
                    delivery_state=OutboundMessageStatus.sent.value,
                    provider_message_id=send_result.external_id if send_result else None,
                )
            _audit(
                db,
                state,
        ...
        *,
        ...
        if ticket is not None:
            ...
            _mark_chat_delivery_state(
                db,
                message=message,
                delivery_state=chat_delivery_state,
                provider_message_id=provider_message_id,
                error=message.last_error,
            )
            chat_message = (
                db.get(ChatMessageRecord, message.chat_message_id)
                if message.chat_message_id
                else None
            )
            if chat_message is not None and normalized_status == "delivered":
                chat_message.delivered_at = receipt_received_at
            if chat_message is not None and normalized_status == "read":
                chat_message.read_at = receipt_received_at
        _audit(
            db,
            state,
            actor=actor,
