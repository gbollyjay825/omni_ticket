...
def oidc_provider_config(config: ResolvedOidcConfig | None = None) -> OidcProviderConfig:
    ...
    return OidcProviderConfig(
        ...
        password_login_available=settings.password_login_mode != "disabled",
        password_login_mode=settings.password_login_mode,
...
def _fetch_userinfo(
    ...
    groups_value = payload.get(settings.oidc_groups_claim)
    if isinstance(groups_value, list):
        groups = [str(value).strip() for value in groups_value if str(value).strip()]
    elif isinstance(groups_value, str) and groups_value.strip():
        groups = [groups_value.strip()]
    else:
        groups = []
    return OidcUserInfo(
        subject=subject,
...
def mapped_access_for_groups(groups: list[str]) -> tuple[UserRole | None, set[str]]:
    """Resolve approved Entra groups into one Omni role and the permitted markets."""

    matched_roles = {
        UserRole(settings.oidc_group_role_map[group_id])
        for group_id in groups
        if group_id in settings.oidc_group_role_map
    }
    role_priority = {
        UserRole.admin: 5,
    ...
    role = max(matched_roles, key=role_priority.__getitem__) if matched_roles else None
    market_ids = {
        market_id
        for group_id in groups
        for market_id in settings.oidc_group_market_map.get(group_id, [])
        if market_id
    }
    return role, market_ids
