...
class Settings(BaseSettings):
    ...
    realtime_channel_prefix: str = "omni:realtime"
    realtime_poll_interval_seconds: float = 1.0
    realtime_backlog_limit: int = 250
    ...
    password_login_mode: Literal["all", "break_glass", "disabled"] = "all"
    break_glass_admin_emails: list[str] = []
    qa_admin_enabled: bool = False
    qa_admin_email: str | None = None
    qa_admin_password: str | None = None
    qa_admin_market_ids: str = "market-ng"
    ...
    freshdesk_base_url: str | None = None
    freshdesk_api_key: str | None = None
    freshchat_base_url: str | None = None
    freshchat_api_token: str | None = None
    freshworks_import_timeout_seconds: int = 30
    freshworks_import_max_retries: int = 5
    ...
    oidc_provider_type: Literal["generic", "microsoft_entra"] = "generic"
    ...
    oidc_group_market_map: dict[str, list[str]] = {}
    ...
    def resolved_session_cookie_path(self) -> str:
        return self.session_cookie_path or self.api_prefix
    ...
    def deployment_errors(self, process_name: str) -> list[str]:
        ...
        if self.database_pool_size < 1:
            errors.append("OMNI_DATABASE_POOL_SIZE must be at least 1.")
        if self.database_max_overflow < 0:
            errors.append("OMNI_DATABASE_MAX_OVERFLOW must be at least 0.")
        if self.database_pool_timeout_seconds < 1:
            errors.append("OMNI_DATABASE_POOL_TIMEOUT_SECONDS must be at least 1.")
        if self.database_pool_recycle_seconds < 1:
            errors.append("OMNI_DATABASE_POOL_RECYCLE_SECONDS must be at least 1.")
        if self.realtime_poll_interval_seconds <= 0:
            errors.append("OMNI_REALTIME_POLL_INTERVAL_SECONDS must be greater than 0.")
        if self.realtime_backlog_limit < 1 or self.realtime_backlog_limit > 1000:
            errors.append("OMNI_REALTIME_BACKLOG_LIMIT must be between 1 and 1000.")
        if self.realtime_presence_ttl_seconds < 10:
            errors.append("OMNI_REALTIME_PRESENCE_TTL_SECONDS must be at least 10.")
        ...
        if not self.session_cookie_name.strip():
            errors.append("OMNI_SESSION_COOKIE_NAME must not be empty.")
        if not self.resolved_session_cookie_path.startswith("/"):
            errors.append("OMNI_SESSION_COOKIE_PATH must start with '/'.")
        normalized_break_glass_emails = {
            value.strip().lower() for value in self.break_glass_admin_emails if value.strip()
        }
        if self.password_login_mode == "break_glass" and not normalized_break_glass_emails:
            errors.append(
                "OMNI_BREAK_GLASS_ADMIN_EMAILS must name at least one administrator when break-glass password login is enabled."
            )
        if any(not value.endswith("@wakanow.com") for value in normalized_break_glass_emails):
            errors.append("OMNI_BREAK_GLASS_ADMIN_EMAILS may contain only wakanow.com identities.")
        if self.qa_admin_enabled:
            normalized_qa_email = (self.qa_admin_email or "").strip().lower()
            qa_market_ids = [
                value.strip() for value in self.qa_admin_market_ids.split(",") if value.strip()
            ]
            if not normalized_qa_email.endswith("@wakanow.com"):
                errors.append("OMNI_QA_ADMIN_EMAIL must be a wakanow.com identity.")
            if not self.qa_admin_password or len(self.qa_admin_password) < 8:
                errors.append("OMNI_QA_ADMIN_PASSWORD must be at least 8 characters.")
            if not qa_market_ids:
                errors.append("OMNI_QA_ADMIN_MARKET_IDS must contain at least one market.")
            if self.production_like:
                errors.append("OMNI_QA_ADMIN_ENABLED is not allowed in staging/production.")
        ...
        if self.freshworks_import_timeout_seconds < 1:
            errors.append("OMNI_FRESHWORKS_IMPORT_TIMEOUT_SECONDS must be at least 1.")
        if not 0 <= self.freshworks_import_max_retries <= 10:
            errors.append("OMNI_FRESHWORKS_IMPORT_MAX_RETRIES must be between 0 and 10.")
        ...
        if not self.oidc_groups_claim.strip():
            errors.append("OMNI_OIDC_GROUPS_CLAIM must not be empty.")
        valid_oidc_roles = {"agent", "supervisor", "admin", "auditor", "service_account"}
        invalid_group_roles = sorted(
            role for role in self.oidc_group_role_map.values() if role not in valid_oidc_roles
        )
        if invalid_group_roles:
            errors.append("OMNI_OIDC_GROUP_ROLE_MAP contains an invalid Omni user role.")
        if any(not group_id.strip() for group_id in self.oidc_group_role_map):
            errors.append("OMNI_OIDC_GROUP_ROLE_MAP contains an empty group identifier.")
        if any(
            not group_id.strip() or not market_ids or any(not market_id.strip() for market_id in market_ids)
            for group_id, market_ids in self.oidc_group_market_map.items()
        ):
            errors.append("OMNI_OIDC_GROUP_MARKET_MAP must map non-empty group identifiers to market identifiers.")
        ...
        if self.production_like:
            ...
            if not self.session_cookie_secure:
                errors.append("OMNI_SESSION_COOKIE_SECURE must be true in staging/production.")
            ...
            if not self.redis_url:
                errors.append("OMNI_REDIS_URL is required in staging/production.")
            if self.attachment_storage_backend != "s3":
                errors.append("OMNI_ATTACHMENT_STORAGE_BACKEND must be s3 in staging/production.")
            if self.attachment_scanner_adapter != "http":
                errors.append("OMNI_ATTACHMENT_SCANNER_ADAPTER must be http in staging/production.")
            if self.password_login_mode != "break_glass":
                errors.append(
                    "OMNI_PASSWORD_LOGIN_MODE must be break_glass in staging/production."
                )
            if not self.oidc_enabled:
                errors.append("OMNI_OIDC_ENABLED must be true in staging/production.")
            if self.oidc_provider_type != "microsoft_entra":
                errors.append(
                    "OMNI_OIDC_PROVIDER_TYPE must be microsoft_entra in staging/production."
                )
            if set(self.oidc_allowed_email_domains) != {"wakanow.com"}:
                errors.append(
                    "OMNI_OIDC_ALLOWED_EMAIL_DOMAINS must contain only wakanow.com in staging/production."
                )
            if self.oidc_auto_provision_enabled:
                errors.append(
                    "OMNI_OIDC_AUTO_PROVISION_ENABLED must be false in staging/production."
                )
            if not self.oidc_require_email_verified:
                errors.append(
                    "OMNI_OIDC_REQUIRE_EMAIL_VERIFIED must be true in staging/production."
                )
            if not self.oidc_group_role_map:
                errors.append("OMNI_OIDC_GROUP_ROLE_MAP is required in staging/production.")
            if not self.oidc_group_market_map:
                errors.append("OMNI_OIDC_GROUP_MARKET_MAP is required in staging/production.")
        return errors
