...
ALEMBIC_HEAD = "20260716_0059"


def test_file_sqlite_uses_unbounded_short_lived_connections(tmp_path: Path) -> None:
    engine = create_database_engine(f"sqlite:///{tmp_path / 'pool-test.db'}")
...
def test_database_schema_and_seed_are_postgres_ready_with_local_sqlite(tmp_path: Path) -> None:
    ...
    assert "ticket_views" in tables
    assert "cases" in tables
    assert "realtime_events" in tables
    assert "chat_conversations" in tables
    assert "chat_messages" in tables
    assert "chat_participants" in tables
    assert "conversation_assignments" in tables
    assert "conversation_views" in tables
    assert "conversation_topics" in tables
    assert "message_receipts" in tables
    assert "conversation_ticket_links" in tables
    assert "conversation_attachments" in tables
    assert "feature_flags" in tables
    assert "personal_tasks" in tables
    assert "ticket_watchers" in tables
    assert "ticket_time_entries" in tables
    assert "customer_identities" in tables
    assert "import_runs" in tables
    assert "import_cursors" in tables
    assert "external_id_mappings" in tables
    assert "import_reconciliations" in tables
    assert "import_cutover_decisions" in tables
    ...
    assert "widget_settings" in tables
...
def test_store_sync_never_recreates_a_user_with_the_fixture_password(tmp_path: Path) -> None:
    database_path = tmp_path / "omni-ticket-store-sync.db"
    engine = create_database_engine(f"sqlite:///{database_path}")
    create_schema(engine)
    ...
    with Session(engine) as session:
        ...
        persist_store_state(session, store)
        session.commit()

        recreated = session.get(UserRecord, user_id)
        assert recreated is not None
        assert recreated.password_reset_required is True
        assert not verify_password("omni-demo", recreated.password_hash)


def test_seed_reference_data_persists_configured_qa_administrator(
    tmp_path: Path,
    monkeypatch,
) -> None:
    database_path = tmp_path / "omni-ticket-qa-admin.db"
    engine = create_database_engine(f"sqlite:///{database_path}")
    create_schema(engine)
    ...
    with Session(engine) as session:
        ...
        records = session.scalars(
            select(UserRecord).where(UserRecord.email == "qa-admin@wakanow.com")
        ).all()
        ...
        assert user.default_market_id == "market-ng"
        assert user.market_ids == ["market-ng", "market-gh"]
        assert verify_password("qa-password-2026", user.password_hash)
...
def test_create_schema_repairs_legacy_user_auth_columns_and_stamps_head(
    ...
    assert "segments" in inspector.get_table_names()
    assert "ai_agents" in inspector.get_table_names()
    assert "service_appointments" in inspector.get_table_names()
    ...
    assert user.password_reset_required == 1
