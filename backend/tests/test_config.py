...
def test_session_cookie_path_must_be_absolute() -> None:
    invalid = Settings(session_cookie_path="api/v1")
    root_scoped = Settings(session_cookie_path="/")

    assert any(
        "OMNI_SESSION_COOKIE_PATH" in error for error in invalid.deployment_errors("web")
    )
    assert root_scoped.deployment_errors("web") == []


def test_local_qa_admin_access_requires_complete_wakanow_configuration() -> None:
    valid = Settings(
        environment="local",
        qa_admin_enabled=True,
        qa_admin_email="qa-admin@wakanow.com",
        qa_admin_password="qa-password-2026",
        qa_admin_market_ids="market-ng,market-gh",
    )
    invalid = Settings(
        environment="local",
        qa_admin_enabled=True,
        qa_admin_email="qa-admin@example.com",
        qa_admin_password="short",
        qa_admin_market_ids="",
    )

    assert valid.deployment_errors("web") == []
    errors = invalid.deployment_errors("web")
    assert any("OMNI_QA_ADMIN_EMAIL" in error for error in errors)
    assert any("OMNI_QA_ADMIN_PASSWORD" in error for error in errors)
    assert any("OMNI_QA_ADMIN_MARKET_IDS" in error for error in errors)


def test_qa_admin_access_is_rejected_in_strict_production() -> None:
    settings = Settings(
    ...
    assert any(
        "OMNI_QA_ADMIN_ENABLED" in error for error in settings.deployment_errors("web")
    )
...
def test_production_settings_require_postgres_explicit_migrations_and_frontend_origin() -> None:
    ...
    assert any("OMNI_SESSION_COOKIE_SECURE" in error for error in errors)
    ...
    assert any("OMNI_REDIS_URL" in error for error in errors)
    ...
    assert any("OMNI_PASSWORD_LOGIN_MODE" in error for error in errors)
    assert any("OMNI_OIDC_ENABLED" in error for error in errors)
    assert any("OMNI_OIDC_PROVIDER_TYPE" in error for error in errors)


def test_valid_production_requires_managed_runtime_boundaries() -> None:
    settings = Settings(
        environment="production",
        ...
        attachment_s3_bucket="omni-production",
        attachment_s3_region="eu-west-1",
        attachment_scanner_adapter="http",
        attachment_scanner_http_endpoint="https://scanner.example.com/scan",
        password_login_mode="break_glass",
        break_glass_admin_emails=["breakglass@wakanow.com"],
        oidc_enabled=True,
        oidc_provider_type="microsoft_entra",
        ...
        oidc_redirect_url="https://omni.wakanow.com/?auth=oidc",
        oidc_group_role_map={"entra-admins": "admin"},
        oidc_group_market_map={"entra-ng": ["market-ng"]},
    )
...
def test_break_glass_and_entra_group_settings_are_validated() -> None:
    settings = Settings(
        password_login_mode="break_glass",
        break_glass_admin_emails=["admin@example.com"],
        oidc_groups_claim="",
        oidc_group_role_map={"group-admin": "owner"},
        oidc_group_market_map={"group-empty": []},
    )

    errors = settings.deployment_errors("web")

    assert any("OMNI_BREAK_GLASS_ADMIN_EMAILS" in error for error in errors)
    assert any("OMNI_OIDC_GROUPS_CLAIM" in error for error in errors)
    assert any("OMNI_OIDC_GROUP_ROLE_MAP" in error for error in errors)
    assert any("OMNI_OIDC_GROUP_MARKET_MAP" in error for error in errors)
