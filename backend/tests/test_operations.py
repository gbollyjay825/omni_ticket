...
from app.db.bootstrap import ensure_qa_admin
...
def test_break_glass_password_login_requires_named_admin_with_mfa(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    anonymous = TestClient(create_app())
    monkeypatch.setattr(settings, "password_login_mode", "break_glass")
    monkeypatch.setattr(
        settings,
        "break_glass_admin_emails",
        ["gbolahan@omniticket.example.com"],
    )

    without_mfa = anonymous.post(
        "/api/v1/auth/login",
        json={"email": "gbolahan@omniticket.example.com", "password": "omni-demo"},
    )
    assert without_mfa.status_code == 403
    assert "MFA" in without_mfa.json()["detail"]

    secret = "JBSWY3DPEHPK3PXP"
    with Session(get_engine()) as session:
        record = session.scalar(
            select(UserRecord).where(UserRecord.email == "gbolahan@omniticket.example.com")
        )
    ...
    approved = anonymous.post(
        "/api/v1/auth/login",
        json={
            "email": "gbolahan@omniticket.example.com",
            "password": "omni-demo",
            "mfa_code": current_totp_code(secret),
        },
    )
    assert approved.status_code == 200

    monkeypatch.setattr(settings, "break_glass_admin_emails", ["other@wakanow.com"])
    not_named = anonymous.post(
        "/api/v1/auth/login",
        json={
            "email": "gbolahan@omniticket.example.com",
            "password": "omni-demo",
    ...
    assert not_named.status_code == 403


def test_browser_session_uses_http_only_cookie_and_logout_revokes_it() -> None:
    ...
    session_token = browser.cookies.get(settings.session_cookie_name)
    assert session_token

    restored = browser.get(
        "/api/v1/auth/browser/session",
        headers={"X-Omni-Market": "market-gh"},
    )
    assert restored.status_code == 200
    assert restored.json()["access_token"] == ""
    assert restored.json()["market"]["id"] == "market-gh"

    logout = browser.post("/api/v1/auth/browser/logout")
    assert logout.status_code == 204
    with Session(get_engine()) as session:
        session_record = session.get(SessionRecord, session_token)
        assert session_record is not None
        assert session_record.revoked_at is not None

    assert browser.get("/api/v1/auth/browser/session").status_code == 401


def test_browser_qa_login_creates_admin_session_only_when_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    browser = TestClient(create_app())
    assert browser.post("/api/v1/auth/browser/qa-login").status_code == 404

    monkeypatch.setattr(settings, "qa_admin_enabled", True)
    monkeypatch.setattr(settings, "qa_admin_email", "qa-admin@wakanow.com")
    monkeypatch.setattr(settings, "qa_admin_password", "qa-password-2026")
    monkeypatch.setattr(settings, "qa_admin_market_ids", "market-ng,market-gh")
    monkeypatch.setattr(settings, "session_cookie_path", "/")
    with Session(get_engine()) as session:
        ensure_qa_admin(session)
        session.commit()

    login_response = browser.post("/api/v1/auth/browser/qa-login")

    assert login_response.status_code == 200
    body = login_response.json()
    assert body["access_token"] == ""
    assert body["user"]["email"] == "qa-admin@wakanow.com"
    assert body["user"]["role"] == "admin"
    assert body["market"]["id"] == "market-ng"
    assert {market["id"] for market in body["available_markets"]} == {
        "market-ng",
        "market-gh",
    }
    assert browser.cookies.get(settings.session_cookie_name)
    assert "Path=/" in login_response.headers["set-cookie"]
    assert browser.get("/api/v1/auth/browser/session").status_code == 200
...
def test_oidc_groups_map_preapproved_user_role_and_market(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _enable_oidc(monkeypatch)
    monkeypatch.setattr(
        settings,
        "oidc_group_role_map",
        {"entra-omni-supervisors": "supervisor"},
    )
    monkeypatch.setattr(
        settings,
        "oidc_group_market_map",
        {"entra-omni-ng": ["market-ng"]},
    )
    monkeypatch.setattr(
        identity_service.urlrequest,
        "urlopen",
        _fake_oidc_urlopen(
            {
                "sub": "wakanow-group-subject",
                "email": "gbolahan@omniticket.example.com",
                "name": "Gbolahan Salami",
                "email_verified": True,
                "groups": ["entra-omni-supervisors", "entra-omni-ng"],
            },
            [],
        ),
    )
    ...
    callback = anonymous.post(
        "/api/v1/auth/oidc/callback",
        json={"code": "group-auth-code", "state": start["state"]},
    )

    assert callback.status_code == 200
    assert callback.json()["user"]["role"] == "supervisor"
    with Session(get_engine()) as session:
        user = session.scalar(
            select(UserRecord).where(UserRecord.email == "gbolahan@omniticket.example.com")
        )
        ...
        assert session.scalar(
            select(AuditEventRecord).where(
                AuditEventRecord.action == "user.oidc.role_mapped",
                AuditEventRecord.entity_id == user.id,
            )
        ) is not None


def test_oidc_group_mapping_rejects_unapproved_market(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _enable_oidc(monkeypatch)
    monkeypatch.setattr(settings, "oidc_group_role_map", {"entra-agent": "agent"})
    monkeypatch.setattr(
        settings,
        "oidc_group_market_map",
        {"entra-gh": ["market-gh"]},
    ...
    monkeypatch.setattr(
        ...
        _fake_oidc_urlopen(
            {
                "sub": "wakanow-wrong-market",
                "email": "gbolahan@omniticket.example.com",
                "name": "Gbolahan Salami",
                "email_verified": True,
                "groups": ["entra-agent", "entra-gh"],
            },
            [],
        ),
    )
    ...
    callback = anonymous.post(
        "/api/v1/auth/oidc/callback",
        json={"code": "wrong-market-code", "state": start["state"]},
    )

    assert callback.status_code == 403
    assert "group access" in callback.json()["detail"].lower()
...
def test_contact_workspace_supports_profile_filters_export_merge_and_soft_delete(
    client: TestClient,
) -> None:
    suffix = uuid4().hex
    target = client.post(
        "/api/v1/customers",
        json={
            "name": f"Target Contact {suffix}",
            "email": f"target-contact-{suffix}@example.com",
            "job_title": "Travel manager",
            "timezone": "Africa/Lagos",
            "language": "English",
            "tags": ["priority-contact"],
            "contact_points": [
                {"channel": "voice", "value": f"+23480{suffix[:7]}", "verified": True}
            ],
        },
    )
    source = client.post(
        ...
        json={
            ...
            "email": f"source-contact-{suffix}@example.com",
            "tags": ["merge-source"],
            "notes": "Preserve this contact note.",
        },
    )
    ...
    phone_search = client.get("/api/v1/customers", params={"q": f"+23480{suffix[:7]}"})
    assert phone_search.status_code == 200
    assert [item["id"] for item in phone_search.json()] == [target_id]
    tag_filter = client.get("/api/v1/customers", params={"tag": "priority-contact"})
    assert any(item["id"] == target_id for item in tag_filter.json())

    exported = client.get("/api/v1/customers/export.csv", params={"q": suffix})
    assert exported.status_code == 200
    assert "text/csv" in exported.headers["content-type"]
    assert f"Target Contact {suffix}" in exported.text
    assert "Travel manager" in exported.text

    ticket = client.post(
        "/api/v1/tickets",
        json={
            "subject": f"Merge contact ticket {suffix}",
            "description": "Move this ticket to the surviving contact.",
    ...
    workspace = client.get(f"/api/v1/customers/{source_id}")
    assert workspace.status_code == 200
    assert workspace.json()["tickets"][0]["ticket"]["id"] == ticket.json()["id"]
    assert isinstance(workspace.json()["tickets"][0]["timeline"], list)

    merged = client.post(f"/api/v1/customers/{source_id}/merge/{target_id}")
    assert merged.status_code == 200
    assert merged.json()["customer"]["id"] == target_id
    assert "merge-source" in merged.json()["customer"]["tags"]
    assert "Preserve this contact note" in merged.json()["customer"]["notes"]
    source_lookup = client.get("/api/v1/customers", params={"q": f"Source Contact {suffix}"})
    assert all(item["id"] != source_id for item in source_lookup.json())
    moved_tickets = client.get("/api/v1/tickets", params={"customer_id": target_id})
    assert any(item["id"] == ticket.json()["id"] for item in moved_tickets.json())

    deleted = client.delete(f"/api/v1/customers/{target_id}")
    assert deleted.status_code == 204
    target_lookup = client.get("/api/v1/customers", params={"q": f"Target Contact {suffix}"})
    assert target_lookup.json() == []
