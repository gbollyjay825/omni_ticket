...
export interface BackendPersonalTask {
  id: string
  market_id: string
  user_id: string
  label: string
...
export interface BackendPersonalWorkspace {
  tasks: BackendPersonalTask[]
  watched_ticket_ids: string[]
}

export interface BackendTicketTimeEntry {
  id: string
  market_id: string
  ticket_id: string
  user_id: string
...
export type BackendReportType = 'tickets' | 'chat' | 'csat' | 'team' | 'ai'
...
export interface BackendPortalMarket {
  code: string
  name: string
  default_locale: string
}

export interface BackendPortalConfiguration {
  market_id: string
  market_code: string
  market_name: string
  default_locale: string
...
export interface BackendWidgetPublicConfiguration {
  market_code: string
  enabled: boolean
  available: boolean
  availability: 'online' | 'disabled' | 'paused' | 'outside_business_hours'
  display_name: string
  welcome_message: string
  primary_color: string
  launcher_label: string
...
export interface BackendWidgetConversationMessage {
  id: string
  actor: string
  body: string
  channel: string
...
export interface BackendWidgetConversation {
  ticket_id: string
  public_id: string
  status: string
  subject: string
  ...
  created_at: string
  updated_at: string
  messages: BackendWidgetConversationMessage[]
  access_token: string | null
  token_expires_at: string | null
}

export interface BackendWidgetConversationStartInput {
  name?: string
  email?: string
  message: string
  subject?: string
...
export interface BackendCustomerListQuery {
  q?: string
  ...
  sort_by?: 'name' | 'email' | 'sentiment' | 'created_at' | 'updated_at'
  sort_order?: 'asc' | 'desc'
  limit?: number
  offset?: number
}
...
export interface BackendCustomerWorkspace {
  customer: BackendCustomer
  company: BackendCompany | null
  tickets: BackendTicketContext[]
}

export interface BackendCustomerMergeResult {
  customer: BackendCustomer
  merged_customer_id: string
}

export interface BackendCustomerImportResult {
  created: number
  skipped: number
...
export interface BackendCreateAutomationRuleInput {
  name: string
  enabled?: boolean
  trigger: string
  action: string
...
export async function loginBackend(input: BackendLoginInput): Promise<BackendSession> {
  return fetchJson<BackendSession>('/auth/browser/login', {
    method: 'POST',
    body: JSON.stringify(input),
  })
}

export async function loginQaAdminBackend(): Promise<BackendSession> {
  return fetchJson<BackendSession>('/auth/browser/qa-login', { method: 'POST' })
}

export async function restoreBackendSession(): Promise<BackendSession> {
  return fetchJson<BackendSession>('/auth/browser/session')
}

export async function logoutBackend(): Promise<void> {
  return fetchJson<void>('/auth/browser/logout', { method: 'POST' })
}
...
export async function completeOidcLoginBackend(
  ...
  return fetchJson<BackendSession>('/auth/browser/oidc/callback', {
    method: 'POST',
    body: JSON.stringify(input),
  })
}
...
export async function exportBackendReport(
  reportType: BackendReportType,
  session: BackendSession,
): Promise<BackendAuditExport> {
  const params = new URLSearchParams({ report_type: reportType })
  const exportPayload = await fetchAuthenticatedText(`/reports/export?${params.toString()}`, session)
  if (exportPayload.filename === 'omni-audit-export.txt') {
    return { ...exportPayload, filename: `omni-${reportType}-report.csv` }
  }
  return exportPayload
}
...
export async function fetchBackendPersonalWorkspace(
  session: BackendSession,
  signal?: AbortSignal,
): Promise<BackendPersonalWorkspace> {
  return fetchJson<BackendPersonalWorkspace>('/me/workspace', { signal }, session)
}

export async function createBackendPersonalTask(
  label: string,
  session: BackendSession,
): Promise<BackendPersonalTask> {
  return fetchJson<BackendPersonalTask>(
    '/me/tasks',
...
export async function patchBackendPersonalTask(
  taskId: string,
  patch: { label?: string; completed?: boolean; position?: number },
  session: BackendSession,
): Promise<BackendPersonalTask> {
  return fetchJson<BackendPersonalTask>(
...
export async function deleteBackendPersonalTask(
  taskId: string,
  session: BackendSession,
): Promise<void> {
  return fetchJson<void>(
...
export async function watchBackendTicket(
  ticketId: string,
  watching: boolean,
  session: BackendSession,
): Promise<void> {
  return fetchJson<void>(
    `/tickets/${encodeURIComponent(ticketId)}/watch`,
    { method: watching ? 'PUT' : 'DELETE' },
    session,
  )
}

export async function fetchBackendTicketTimeEntries(
  ticketId: string,
  session: BackendSession,
  signal?: AbortSignal,
): Promise<BackendTicketTimeEntry[]> {
  return fetchJson<BackendTicketTimeEntry[]>(
    `/tickets/${encodeURIComponent(ticketId)}/time-entries`,
    { signal },
    session,
  )
}

export async function createBackendTicketTimeEntry(
  ticketId: string,
  input: { minutes: number; note?: string; billable?: boolean },
  session: BackendSession,
): Promise<BackendTicketTimeEntry> {
  return fetchJson<BackendTicketTimeEntry>(
    `/tickets/${encodeURIComponent(ticketId)}/time-entries`,
...
export async function deleteBackendTicketTimeEntry(
  ticketId: string,
  entryId: string,
  session: BackendSession,
): Promise<void> {
  return fetchJson<void>(
    `/tickets/${encodeURIComponent(ticketId)}/time-entries/${encodeURIComponent(entryId)}`,
    { method: 'DELETE' },
    session,
  )
}
...
export async function fetchBackendPortalMarkets(): Promise<BackendPortalMarket[]> {
  return fetchJson<BackendPortalMarket[]>('/portal/markets')
}

export async function fetchBackendPortalConfiguration(
  marketCode: string,
): Promise<BackendPortalConfiguration> {
  return fetchJson<BackendPortalConfiguration>(
    `/portal/${encodeURIComponent(marketCode.toLowerCase())}/config`,
  )
}

export async function fetchBackendWidgetConfiguration(
  marketCode: string,
): Promise<BackendWidgetPublicConfiguration> {
  return fetchJson<BackendWidgetPublicConfiguration>(
    `/widget/${encodeURIComponent(marketCode.toLowerCase())}/config`,
  )
}

export async function createBackendWidgetConversation(
  marketCode: string,
  input: BackendWidgetConversationStartInput,
): Promise<BackendWidgetConversation> {
  return fetchJson<BackendWidgetConversation>(
    `/widget/${encodeURIComponent(marketCode.toLowerCase())}/conversations`,
    { method: 'POST', body: JSON.stringify(input) },
  )
}

export async function fetchBackendWidgetConversation(
  marketCode: string,
  publicId: string,
  accessToken: string,
): Promise<BackendWidgetConversation> {
  return fetchJson<BackendWidgetConversation>(
    `/widget/${encodeURIComponent(marketCode.toLowerCase())}/conversations/${encodeURIComponent(publicId)}`,
    { headers: { Authorization: `Widget ${accessToken}` } },
  )
}

export async function createBackendWidgetMessage(
  marketCode: string,
  publicId: string,
  accessToken: string,
  body: string,
  ...
  return fetchJson<BackendWidgetConversation>(
    `/widget/${encodeURIComponent(marketCode.toLowerCase())}/conversations/${encodeURIComponent(publicId)}/messages`,
    {
      method: 'POST',
      headers: { Authorization: `Widget ${accessToken}` },
...
export async function fetchBackendCustomerPage(
  query: BackendCustomerListQuery,
  session: BackendSession,
  signal?: AbortSignal,
): Promise<BackendCustomerPage> {
  const params = new URLSearchParams()
  Object.entries(query).forEach(([key, value]) => {
    if (value !== undefined && value !== '') params.set(key, String(value))
  })
  const headers = new Headers(authHeaders(session))
  const response = await fetch(`${getBackendBaseUrl()}/customers?${params.toString()}`, {
    headers,
    credentials: 'include',
    signal,
  })
  ...
  const items = (await response.json()) as BackendCustomer[]
  return {
    items,
    total: Number(response.headers.get('X-Total-Count') ?? items.length),
    returned: Number(response.headers.get('X-Returned-Count') ?? items.length),
    limit: Number(response.headers.get('X-Limit') ?? query.limit ?? items.length),
    offset: Number(response.headers.get('X-Offset') ?? query.offset ?? 0),
  }
}

export async function fetchBackendCustomerWorkspace(
  customerId: string,
  session: BackendSession,
): Promise<BackendCustomerWorkspace> {
  return fetchJson<BackendCustomerWorkspace>(`/customers/${customerId}`, undefined, session)
}

export async function exportBackendCustomers(
  query: BackendCustomerListQuery,
  session: BackendSession,
): Promise<BackendAuditExport> {
  const params = new URLSearchParams()
  ...
  return fetchAuthenticatedText(`/customers/export.csv?${params.toString()}`, session)
}

export async function importBackendCustomersCsv(
  csv: string,
  session: BackendSession,
): Promise<BackendCustomerImportResult> {
  const headers = new Headers(authHeaders(session))
  headers.set('Content-Type', 'text/csv; charset=utf-8')
  const response = await fetch(`${getBackendBaseUrl()}/customers/import`, {
    method: 'POST',
    headers,
    credentials: 'include',
    body: csv,
  ...
  return response.json() as Promise<BackendCustomerImportResult>
}
...
export async function deleteBackendCustomer(
  customerId: string,
  session: BackendSession,
): Promise<void> {
  return fetchJson<void>(`/customers/${customerId}`, { method: 'DELETE' }, session)
}

export async function mergeBackendCustomers(
  sourceCustomerId: string,
  targetCustomerId: string,
  session: BackendSession,
): Promise<BackendCustomerMergeResult> {
  return fetchJson<BackendCustomerMergeResult>(
    `/customers/${sourceCustomerId}/merge/${targetCustomerId}`,
    { method: 'POST' },
    session,
  )
}
...
export async function createBackendAutomationRule(
  input: BackendCreateAutomationRuleInput,
  session: BackendSession,
): Promise<BackendAutomationRule> {
  return fetchJson<BackendAutomationRule>(
    '/automation-rules',
