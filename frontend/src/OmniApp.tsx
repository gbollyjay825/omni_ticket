...
import {
  createBackendPersonalTask,
  createBackendAutomationRule,
  createBackendTicketTimeEntry,
  createBackendDiscussionComment,
  deleteBackendCustomer,
  deleteBackendPersonalTask,
  deleteBackendTicketTimeEntry,
  fetchBackendDiscussionComments,
  fetchBackendCustomerPage,
  fetchBackendCustomerWorkspace,
  fetchBackendPersonalWorkspace,
  fetchBackendTicketTimeEntries,
  exportBackendAudit,
  exportBackendCustomers,
  exportBackendReport,
...
import { useOmnichatRealtime } from './features/omnichat/useOmnichatRealtime'
import type {
  SchedulingCreateInput,
  SchedulingUpdateInput,
} from './features/workforce/SchedulingWorkspace'
...
type CustomerDetailTab = 'timeline' | 'tickets' | 'notes' | 'forums'

const CUSTOMER_PAGE_SIZE = 30

const LOCAL_ADMIN_ACCESS = import.meta.env.DEV
  ? {
...
const QA_ADMIN_EMAIL = import.meta.env.VITE_OMNI_QA_ADMIN_EMAIL?.trim()
const QA_ADMIN_ACCESS = QA_ADMIN_EMAIL
  ? {
      email: QA_ADMIN_EMAIL,
      marketId: import.meta.env.VITE_OMNI_QA_ADMIN_MARKET_ID?.trim() || 'market-ng',
    }
  : null

const ImportControlPlane = lazy(() =>
  import('./features/admin/ImportControlPlane').then((module) => ({
    default: module.ImportControlPlane,
  })),
)
const ReportCatalog = lazy(() =>
  import('./features/analytics/ReportCatalog').then((module) => ({
    default: module.ReportCatalog,
  })),
)
const ReportDeliveryHistory = lazy(() =>
  import('./features/analytics/ReportDeliveryHistory').then((module) => ({
    default: module.ReportDeliveryHistory,
  })),
)
const CampaignConsole = lazy(() =>
  import('./features/omnichat/CampaignConsole').then((module) => ({
    default: module.CampaignConsole,
  })),
)
const PeopleConsole = lazy(() =>
  import('./features/omnichat/PeopleConsole').then((module) => ({
    default: module.PeopleConsole,
  })),
)
const AiStudioConsole = lazy(() =>
  import('./features/omnichat/AiStudioConsole').then((module) => ({
    default: module.AiStudioConsole,
  })),
)
const OmnichatSettingsConsole = lazy(() =>
  import('./features/omnichat/OmnichatSettingsConsole').then((module) => ({
    default: module.OmnichatSettingsConsole,
  })),
)
const OmnichatWorkspace = lazy(() =>
  import('./features/omnichat/OmnichatWorkspace').then((module) => ({
    default: module.OmnichatWorkspace,
  })),
)
const TicketWorkspace = lazy(() =>
  import('./features/tickets/TicketWorkspace').then((module) => ({
    default: module.TicketWorkspace,
  })),
)
const KnowledgeWorkspace = lazy(() =>
  import('./features/knowledge/KnowledgeWorkspace').then((module) => ({
    default: module.KnowledgeWorkspace,
  })),
)
const SchedulingWorkspace = lazy(() =>
  import('./features/workforce/SchedulingWorkspace').then((module) => ({
    default: module.SchedulingWorkspace,
  })),
)
const WorkflowsWorkspace = lazy(() =>
  import('./features/workflows/WorkflowsWorkspace').then((module) => ({
    default: module.WorkflowsWorkspace,
  })),
)
const ForumsWorkspace = lazy(() =>
  import('./features/forums/ForumsWorkspace').then((module) => ({
    default: module.ForumsWorkspace,
  })),
)
const AiAgentsWorkspace = lazy(() =>
  import('./features/ai/AiAgentsWorkspace').then((module) => ({
    default: module.AiAgentsWorkspace,
  })),
)
const CampaignsWorkspace = lazy(() =>
  import('./features/campaigns/CampaignsWorkspace').then((module) => ({
    default: module.CampaignsWorkspace,
  })),
)
const CustomObjectsWorkspace = lazy(() =>
  import('./features/custom-objects/CustomObjectsWorkspace').then((module) => ({
    default: module.CustomObjectsWorkspace,
  })),
)
...
const screenConfig: { id: ScreenId; label: string; icon: LucideIcon }[] = [
  ...
  { id: 'ai-agents', label: 'AI Agents & Chatbots', icon: Bot },
  ...
  { id: 'campaigns', label: 'WhatsApp Campaigns', icon: Send },
  { id: 'custom-objects', label: 'Custom Objects', icon: DatabaseZap },
...
const screenLead: Record<ScreenId, string> = {
  ...
  forums: 'Community activity, categories, moderation queues, topics, and replies.',
  'ai-agents': 'AI agent configuration, provider readiness, policy, deployment, and audited decisions.',
  campaigns: 'Consent-aware proactive outreach, approved templates, delivery status, and provider readiness.',
  'custom-objects': 'Structured support data schemas and fields for market-specific operational records.',
...
const setupSectionDescriptions: Record<(typeof setupSectionOptions)[number]['id'], string> = {
  people: 'Agents, access, roles, and working hours',
  forms: 'Ticket, contact, product, and portal structure',
  governance: 'Audit, retention, migration, and operations',
  connectors: 'Customer channels and provider accounts',
  automation: 'Routing, SLAs, notifications, and productivity',
}
...
const setupModuleCatalog: Record<SetupSectionId, { title: string; modules: string[] }[]> = {
  ...
  governance: [
    { title: 'Operations', modules: ['Audit export', 'Data migration', 'Retention', 'Attachment lifecycle', 'Operational alerts', 'Scheduled exports'] },
...
function OmniApp() {
  ...
  const liveTypingTimerRef = useRef<number | undefined>(undefined)
  const liveTypingConversationRef = useRef('')
  const liveTypingActiveRef = useRef(false)
  ...
  const [timeLogs, setTimeLogs] = useState<Record<string, TicketTimeLog[]>>({})
  ...
  const [analyticsReportSort, setAnalyticsReportSort] = useState<'modified' | 'name'>('modified')
  const [reportDraft, setReportDraft] = useState<{
    name: string
    reportType: BackendReportType
    recipients: string
  }>({ name: '', reportType: 'tickets', recipients: '' })
  const [reportBusy, setReportBusy] = useState(false)
  const [reportExportBusy, setReportExportBusy] = useState<BackendReportType | null>(null)
  const [reportRecipientDrafts, setReportRecipientDrafts] = useState<Record<string, string>>({})
  ...
  const [customerPageData, setCustomerPageData] = useState<BackendCustomerPage>({
    items: [],
    total: 0,
    returned: 0,
  ...
  const [customerListBusy, setCustomerListBusy] = useState(false)
  const [customerListRevision, setCustomerListRevision] = useState(0)
  const [customerFiltersOpen, setCustomerFiltersOpen] = useState(true)
  const [customerViewsOpen, setCustomerViewsOpen] = useState(false)
  const [customerCompanyFilter, setCustomerCompanyFilter] = useState('')
  const [customerTagFilter, setCustomerTagFilter] = useState('')
  const [customerTimezoneFilter, setCustomerTimezoneFilter] = useState('')
  const [customerCreatedFilter, setCustomerCreatedFilter] = useState('any')
  const [selectedCustomerIds, setSelectedCustomerIds] = useState<string[]>([])
  const [customerDetailTab, setCustomerDetailTab] = useState<CustomerDetailTab>('timeline')
  const [customerWorkspace, setCustomerWorkspace] = useState<BackendCustomerWorkspace | null>(null)
  const [customerWorkspaceBusy, setCustomerWorkspaceBusy] = useState(false)
  ...
  const [customerMergeTargetId, setCustomerMergeTargetId] = useState('')
  ...
  const customerImportInputRef = useRef<HTMLInputElement>(null)
  const [loginEmail, setLoginEmail] = useState(
    LOCAL_ADMIN_ACCESS?.email ?? QA_ADMIN_ACCESS?.email ?? '',
  )
  const [loginPassword, setLoginPassword] = useState(LOCAL_ADMIN_ACCESS?.password ?? '')
  const [loginMfaCode, setLoginMfaCode] = useState('')
  const [loginMarket, setLoginMarket] = useState(
    LOCAL_ADMIN_ACCESS?.marketId ?? QA_ADMIN_ACCESS?.marketId ?? 'market-ng',
  )
  ...
  const customerDetailRouteActive =
    state.selectedScreen === 'customers' &&
    typeof window !== 'undefined' &&
    Boolean(new URLSearchParams(window.location.search).get('customer'))
  ...
  const activeDirectChannelId = isDirectMessageChannel(state.selectedChannelId)
    ...
    : 'whatsapp'
  const activeDirectConversations = state.conversations
    .filter(
      (conversation) =>
        conversation.channelId === activeDirectChannelId &&
  ...
  const activeDirectConversation =
    activeDirectConversations.find((conversation) => conversation.id === selectedConversation.id) ??
    activeDirectConversations[0]
  const latestDirectInboundMessage = activeDirectConversation
    ? [...activeDirectConversation.timeline]
        .reverse()
        .find((event) => event.authorRole === 'customer')
    : undefined
  const extractedWorkspace =
    state.selectedScreen === 'channels' || state.selectedScreen === 'inbox'
  useEffect(() => {
    if (extractedWorkspace || !backendSession || !online) return
    const controller = new AbortController()
    fetchBackendPersonalWorkspace(backendSession, controller.signal)
      .then((workspace) => {
        setTodos(
        ...
        setWatchedTicketIds(workspace.watched_ticket_ids)
      })
      .catch((error) => {
        if (controller.signal.aborted) return
        setUiNotice(
          error instanceof Error ? error.message : 'Personal workspace failed to load.',
        )
      })
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    fetchBackendTicketTimeEntries(selectedConversation.id, backendSession, controller.signal)
      .then((entries) => {
        setTimeLogs((current) => ({
          ...current,
          [selectedConversation.id]: entries.map((entry) => ({
            id: entry.id,
      ...
      .catch((error) => {
        if (controller.signal.aborted) return
        setUiNotice(error instanceof Error ? error.message : 'Time entries failed to load.')
      })
    return () => controller.abort()
  }, [backendSession, extractedWorkspace, online, selectedConversation.id])

  const refreshBackendRef = useRef(refreshBackend)
  const realtimeRefreshTimerRef = useRef<number | undefined>(undefined)
  useEffect(() => {
    refreshBackendRef.current = refreshBackend
  })
  const {
    connectionStatus: omnichatConnectionStatus,
  ...
  useEffect(() => {
    if (
      state.selectedScreen !== 'channels' ||
      channelConsoleView !== 'inbox' ||
      !activeDirectConversation ||
      !latestDirectInboundMessage
    ) {
      return
    }
    markRead(activeDirectConversation.id, latestDirectInboundMessage.id)
  }, [
    activeDirectConversation,
    channelConsoleView,
    latestDirectInboundMessage,
  ...
  useEffect(() => {
    if (state.selectedScreen !== 'customers' || customerDetailRouteActive || !backendSession || !online) {
      return
    }
    const controller = new AbortController()
    const createdSince = (() => {
      const now = new Date()
      if (customerCreatedFilter === '24h') now.setDate(now.getDate() - 1)
      else if (customerCreatedFilter === '7d') now.setDate(now.getDate() - 7)
      else if (customerCreatedFilter === '30d') now.setDate(now.getDate() - 30)
      else return undefined
      return now.toISOString()
    })()
    const query: BackendCustomerListQuery = {
      q: state.filters.search.trim() || undefined,
      company_id: customerCompanyFilter || undefined,
      tag: customerTagFilter.trim() || undefined,
      timezone: customerTimezoneFilter || undefined,
      created_since: createdSince,
      sort_by: 'name',
      sort_order: 'asc',
      limit: CUSTOMER_PAGE_SIZE,
      offset: customerPage * CUSTOMER_PAGE_SIZE,
    }
    queueMicrotask(() => {
      if (!controller.signal.aborted) setCustomerListBusy(true)
    })
    fetchBackendCustomerPage(query, backendSession, controller.signal)
      .then((page) => setCustomerPageData(page))
      .catch((error) => {
        if (controller.signal.aborted) return
        setUiNotice(error instanceof Error ? error.message : 'Contacts failed to load.')
      })
      .finally(() => {
        if (!controller.signal.aborted) setCustomerListBusy(false)
      })
    return () => controller.abort()
  }, [
    backendSession,
    ...
    customerDetailRouteActive,
    customerListRevision,
    customerPage,
    customerTagFilter,
    customerTimezoneFilter,
    online,
    state.filters.search,
    state.selectedScreen,
  ])
  useEffect(() => {
    if (!customerDetailRouteActive || !backendSession || !online || !selectedCustomer.id) return
    let cancelled = false
    queueMicrotask(() => {
      if (!cancelled) setCustomerWorkspaceBusy(true)
    })
    fetchBackendCustomerWorkspace(selectedCustomer.id, backendSession)
      .then((workspace) => {
        if (!cancelled) setCustomerWorkspace(workspace)
      })
      .catch((error) => {
        if (!cancelled) {
          setUiNotice(error instanceof Error ? error.message : 'Contact details failed to load.')
        }
      })
      .finally(() => {
        if (!cancelled) setCustomerWorkspaceBusy(false)
      })
    return () => {
      cancelled = true
    }
  }, [backendSession, customerDetailRouteActive, customerListRevision, online, selectedCustomer.id])
  useEffect(
    () => () => {
      window.clearTimeout(liveTypingTimerRef.current)
      if (liveTypingConversationRef.current && liveTypingActiveRef.current) {
        sendTyping(liveTypingConversationRef.current, false)
      }
    },
    [sendTyping],
  )
  useEffect(() => {
    if (
      !backendSession ||
      !online ||
      !['command', 'analytics'].includes(state.selectedScreen)
    ) {
      return
    }
    const controller = new AbortController()
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (action) {
        event.preventDefault()
        void ticketActionRef.current(action.id)
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setUiNotice(''), 2600)
  ...
  useEffect(() => {
    let cancelled = false
    if (!backendSession || state.selectedScreen !== 'admin') {
      return undefined
    }
  ...
  }, [backendSession, backendSync.lastSyncAt, state.selectedScreen])

  useEffect(() => {
    let cancelled = false
    if (!backendSession || state.selectedScreen !== 'admin' || setupSection !== 'connectors') {
      return undefined
    }
  ...
  }, [backendSession, backendSync.lastSyncAt, setupSection, state.selectedScreen])
  ...
  useEffect(() => {
    ...
    if (extractedWorkspace || !backendSession || query.length < 2) {
      return undefined
    }
  ...
  }, [backendSession, extractedWorkspace, state.filters.search])

  function downloadTextExport(content: string, contentType: string, filename: string) {
  ...
  async function handleAuditExport(format: BackendAuditExportFormat) {
    ...
    try {
      ...
      downloadTextExport(exportPayload.content, exportPayload.contentType, exportPayload.filename)
      setUiNotice(`Audit ${format.toUpperCase()} export downloaded.`)
      const policy = await fetchBackendAuditRetentionPolicy(backendSession)
      setAuditRetention(policy)
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Audit export failed.')
    } finally {
  ...
  async function handleAuditPrune() {
    ...
    try {
      ...
      setUiNotice(`${result.deleted_events} audit event(s) pruned by retention policy.`)
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Audit retention prune failed.')
    } finally {
      setAuditActionBusy(false)
    }
  }
  ...
  async function handleAttachmentPrune() {
    ...
    try {
      ...
      setUiNotice(`${result.purged_attachments} attachment(s) purged by retention policy.`)
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Attachment retention prune failed.')
    } finally {
  ...
  async function handleSsoSettingsSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setUiNotice('Enterprise SSO settings saved.')
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'SSO settings save failed.')
    } finally {
  ...
  async function handleBrandingSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setUiNotice('Portal branding saved.')
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Branding save failed.')
    } finally {
  ...
  async function handleWidgetSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setUiNotice('Chat widget settings saved.')
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Widget save failed.')
    } finally {
      setWidgetBusy(false)
    }
  }
  ...
  async function handleIntegrationCredentialSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setUiNotice('Production credentials saved.')
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Credential save failed.')
    } finally {
  ...
  async function handleProductionAccountRefresh() {
    ...
    try {
      ...
      setUiNotice('Account request pack refreshed.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Account request refresh failed.')
    } finally {
  ...
  async function handleProductionAccountCopy() {
    if (!productionAccountPack) {
      setUiNotice('Account request pack is not loaded yet.')
      return
    }
    try {
      await navigator.clipboard.writeText(productionAccountPack.body)
      setUiNotice('Account request body copied.')
    } catch {
      setUiNotice('Clipboard unavailable; open the email draft instead.')
    }
  }
  ...
  async function handleProductionAccountSend() {
    ...
    try {
      ...
      setUiNotice(error instanceof Error ? error.message : 'Account request email queue failed.')
    } finally {
  ...
  async function handleProductionReadinessRefresh() {
    ...
    try {
      const checklist = await fetchBackendProductionReadinessChecklist(backendSession)
      setProductionReadinessChecklist(checklist)
      setUiNotice('Production readiness checklist refreshed.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Production readiness refresh failed.')
    } finally {
  ...
  async function handleProductionAccountReferenceSave(event: FormEvent) {
    ...
    if (!productionAccountReferenceDraft.accountName.trim()) {
      setUiNotice('Account name is required.')
      return
    }
    ...
    try {
      ...
      setUiNotice('Production account reference saved.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Account reference save failed.')
    } finally {
  ...
  async function handleProductionAccountReferenceStatus(
    ...
    try {
      ...
      await refreshProductionAccountReferences()
      setUiNotice(`${reference.account_name} marked ${titleCase(status)}.`)
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Account reference update failed.')
    } finally {
      setProductionAccountReferenceBusy(false)
    }
  }
  ...
  async function handleProductionAccountReferenceDocsCopy() {
    ...
    if (!markdown) {
      setUiNotice('No account reference snippet is available yet.')
      return
    }
    try {
      await navigator.clipboard.writeText(markdown)
      setUiNotice('Account reference snippet copied.')
    } catch {
      setUiNotice('Clipboard unavailable.')
    }
  }
  ...
  async function handleCreateUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('User saved with a temporary password.')
      }
    } finally {
  ...
  async function handleCreateSupportGroup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Support group saved.')
      }
    } finally {
  ...
  async function handleToggleSupportGroup(groupId: string, active: boolean) {
    ...
    try {
      ...
      if (saved) {
        setUiNotice(`Support group ${active ? 'activated' : 'paused'}.`)
      }
    } finally {
      setGroupActionBusy(false)
    }
  ...
  async function handleToggleSlaPolicy(policyId: string, active: boolean) {
    ...
    try {
      ...
      if (saved) {
        setUiNotice(`SLA policy ${active ? 'activated' : 'paused'}.`)
      }
    } finally {
      setSlaPolicyActionBusy(false)
    }
  ...
  async function handleCreateTicketField(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      setUiNotice('Select fields need at least one option.')
      return
    }
    ...
    setUiNotice('Ticket field saved.')
  }
  ...
  async function handleChangePassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Your password was updated.')
      }
    } finally {
      setUserActionBusy(false)
    }
  ...
  async function handleStartMfaEnrollment() {
    ...
    setMfaConfirmCode('')
    setUiNotice('MFA enrollment started.')
  }
  ...
  async function handleConfirmMfa(event: FormEvent<HTMLFormElement>) {
    ...
    setMfaConfirmCode('')
    setUiNotice('MFA is now enabled.')
  }
  ...
  async function handleOperationalAlertUpdate(
    ...
    if (saved) {
      setUiNotice(`Operational alert ${titleCase(status).toLowerCase()}.`)
    }
  }
    ...
    return (
      <main className="login-shell">
        <section className="login-panel">
          ...
          {LOCAL_ADMIN_ACCESS ? (
            <section className="login-local-access" aria-label="Local development access">
              <div className="login-local-access-heading">
                <Wrench size={17} />
                <div>
                  <strong>Local development access</strong>
                  <span>Persistent admin shortcut for building and testing.</span>
                </div>
              </div>
              <dl>
                <div><dt>Email</dt><dd>{LOCAL_ADMIN_ACCESS.email}</dd></div>
                <div><dt>Password</dt><dd>{LOCAL_ADMIN_ACCESS.password}</dd></div>
                <div><dt>Market</dt><dd>Nigeria</dd></div>
              </dl>
              <button
                type="button"
                disabled={backendSync.status === 'syncing'}
                onClick={() => {
                  setLoginEmail(LOCAL_ADMIN_ACCESS.email)
                  setLoginPassword(LOCAL_ADMIN_ACCESS.password)
                  setLoginMarket(LOCAL_ADMIN_ACCESS.marketId)
                  setLoginMfaCode('')
                  login({
                    email: LOCAL_ADMIN_ACCESS.email,
                    password: LOCAL_ADMIN_ACCESS.password,
                    market_id: LOCAL_ADMIN_ACCESS.marketId,
                  })
                }}
              >
                <Lock size={16} />
          ...
          {QA_ADMIN_ACCESS ? (
            <section className="login-local-access login-qa-access" aria-label="Vercel QA access">
              <div className="login-local-access-heading">
                <UserCheck size={17} />
                <div>
                  <strong>QA administrator access</strong>
                  <span>Shared test identity for the deployed Vercel workspace.</span>
                </div>
              </div>
              <dl>
                <div><dt>Email</dt><dd>{QA_ADMIN_ACCESS.email}</dd></div>
                <div><dt>Market</dt><dd>Nigeria</dd></div>
              ...
              <button
                ...
                disabled={backendSync.status === 'syncing'}
                onClick={() => {
                  setLoginEmail(QA_ADMIN_ACCESS.email)
                  setLoginMarket(QA_ADMIN_ACCESS.marketId)
                  setLoginPassword('')
                  setLoginMfaCode('')
                  loginQaAdmin()
                }}
              >
                <Lock size={16} />
                Sign in as QA administrator
              </button>
            </section>
          ) : null}
          <div className="login-sso-panel">
            <div>
              <strong>{oidcProviderConfig?.provider_name ?? 'Microsoft Entra ID'}</strong>
          ...
          {oidcProviderConfig?.password_login_available !== false ? <form
          ...
          >
            {oidcProviderConfig?.password_login_mode === 'break_glass' ? (
              <strong>Break-glass administrator</strong>
            ) : null}
            <label>
              Email
              <input
  ...
  async function addTodo() {
    ...
    if (!backendSession || !online) {
      announcePrototype('Connect to the Omni backend before adding a task.')
      return
    }
    try {
      const task = await createBackendPersonalTask(label, backendSession)
      setTodos((current) => [
        ...current,
        { id: task.id, label: task.label, done: task.completed },
      ])
      setTodoDraft('')
    } catch (error) {
      announcePrototype(error instanceof Error ? error.message : 'Task could not be added.')
    }
  }

  async function toggleTodo(id: string) {
    const currentTask = todos.find((todo) => todo.id === id)
    if (!currentTask || !backendSession || !online) {
      announcePrototype('Connect to the Omni backend before updating a task.')
      return
    }
    try {
      const task = await patchBackendPersonalTask(
        id,
      ...
      setTodos((current) =>
        current.map((todo) =>
          todo.id === id ? { id: task.id, label: task.label, done: task.completed } : todo,
        ),
      )
    } catch (error) {
      announcePrototype(error instanceof Error ? error.message : 'Task could not be updated.')
    }
  }

  async function removeTodo(id: string) {
    if (!backendSession || !online) {
      announcePrototype('Connect to the Omni backend before removing a task.')
      return
    }
    try {
      await deleteBackendPersonalTask(id, backendSession)
      setTodos((current) => current.filter((todo) => todo.id !== id))
    } catch (error) {
      announcePrototype(error instanceof Error ? error.message : 'Task could not be removed.')
    }
  }
  ...
  async function addTimeLog(ticketId: string) {
    const minutes = Number.parseInt(timeLogDraft, 10)
    if (!Number.isFinite(minutes) || minutes <= 0) return
    if (!backendSession || !online) {
      announcePrototype('Connect to the Omni backend before logging time.')
      return
    }
    try {
      const entry = await createBackendTicketTimeEntry(ticketId, { minutes }, backendSession)
      setTimeLogs((current) => ({
        ...current,
        [ticketId]: [
          ...(current[ticketId] ?? []),
          { id: entry.id, minutes: entry.minutes, agent: entry.agent, at: entry.created_at },
        ],
      }))
      setTimeLogDraft('')
    } catch (error) {
      announcePrototype(error instanceof Error ? error.message : 'Time could not be logged.')
    }
  }

  async function removeTimeLog(ticketId: string, id: string) {
    if (!backendSession || !online) {
      announcePrototype('Connect to the Omni backend before removing a time entry.')
      return
    }
    try {
      await deleteBackendTicketTimeEntry(ticketId, id, backendSession)
      setTimeLogs((current) => ({
        ...current,
        [ticketId]: (current[ticketId] ?? []).filter((log) => log.id !== id),
      }))
    } catch (error) {
      announcePrototype(error instanceof Error ? error.message : 'Time entry could not be removed.')
    }
  }
  ...
  async function handleCreateBusinessHours(event: FormEvent<HTMLFormElement>) {
    ...
    if (saved) {
      ...
      setUiNotice('Business hours saved.')
    }
  }
  ...
  async function toggleBusinessHoursActive(calendar: BusinessHours) {
    ...
    if (saved) setUiNotice(`Business hours ${calendar.active ? 'paused' : 'activated'}.`)
  }
  ...
  async function saveBusinessHoursSchedule(calendar: BusinessHours) {
    ...
    if (saved) {
      ...
      setUiNotice('Business hours schedule saved.')
    }
  }
  ...
  async function handleCreateCannedResponse(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Canned response saved.')
      }
    } finally {
      setCannedBusy(false)
    }
  ...
  async function toggleCannedResponse(macro: ResponseMacro) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Canned response ${macro.active ? 'paused' : 'activated'}.`)
    } finally {
      setCannedBusy(false)
    }
  }
  ...
  async function saveCannedEdit(macroId: string) {
    ...
    try {
      ...
      if (saved) {
        setCannedEditId('')
        setUiNotice('Canned response updated.')
      }
    } finally {
      setCannedBusy(false)
    }
  ...
  async function handleCreateTicketTemplate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Ticket template saved.')
      }
    } finally {
  ...
  async function toggleTicketTemplate(template: TicketTemplate) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Ticket template ${template.active ? 'paused' : 'activated'}.`)
    } finally {
      setTemplateBusy(false)
    }
  }
  ...
  async function toggleTag(tag: Tag) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Tag ${tag.active ? 'paused' : 'activated'}.`)
    } finally {
      setTagBusy(false)
    }
  }
  ...
  async function toggleCsatSurvey(survey: CsatSurvey) {
    ...
    try {
      ...
      if (saved) setUiNotice(`CSAT survey ${survey.active ? 'paused' : 'activated'}.`)
    } finally {
      setSurveyBusy(false)
    }
  }
  ...
  async function handleCreateEmailNotification(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Email notification saved.')
      }
    } finally {
      setNotifBusy(false)
    }
  ...
  async function toggleEmailNotification(notification: EmailNotification) {
    ...
    try {
      ...
      if (saved)
        setUiNotice(`Email notification ${notification.active ? 'paused' : 'activated'}.`)
    } finally {
      setNotifBusy(false)
    }
  }
  ...
  async function handleCreateScenarioAutomation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setScenarioActions([])
        setUiNotice('Scenario automation saved.')
      }
    } finally {
  ...
  async function toggleScenarioAutomation(scenario: ScenarioAutomation) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Scenario automation ${scenario.active ? 'paused' : 'activated'}.`)
    } finally {
      setScenarioBusy(false)
    }
  }
  ...
  async function handleCreateCustomField(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Custom field saved.')
      }
    } finally {
  ...
  async function toggleCustomField(field: CustomFieldDefinition) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Custom field ${field.active ? 'paused' : 'activated'}.`)
    } finally {
      setCustomFieldBusy(false)
    }
  }
  ...
  async function handleCreateCustomObject(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setObjectFields([])
        setUiNotice('Custom object saved.')
      }
    } finally {
      setObjectBusy(false)
    }
  ...
  async function toggleCustomObject(object: CustomObject) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Custom object ${object.active ? 'paused' : 'activated'}.`)
    } finally {
      setObjectBusy(false)
    }
  }
  ...
  async function toggleProduct(product: Product) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Product ${product.active ? 'paused' : 'activated'}.`)
    } finally {
      setProductBusy(false)
    }
  }

  async function handleReportExport(reportType: BackendReportType) {
    if (!backendSession || reportExportBusy) return
    setReportExportBusy(reportType)
    try {
      const exportPayload = await exportBackendReport(reportType, backendSession)
      downloadTextExport(exportPayload.content, exportPayload.contentType, exportPayload.filename)
      setUiNotice(`${titleCase(reportType)} report downloaded.`)
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Report export failed.')
    } finally {
  ...
  async function handleCreateSavedReport(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const recipients = reportDraft.recipients
        ...
        .filter(Boolean)
      const scheduled = analyticsReportGroup === 'scheduled'
      const saved = await createSavedReport({
        name,
        report_type: reportDraft.reportType,
        cadence: scheduled ? 'daily' : 'none',
        recipients,
      })
      if (saved) {
        setReportDraft({ name: '', reportType: 'tickets', recipients: '' })
        setUiNotice('Saved report created.')
      }
    } finally {
      setReportBusy(false)
    }
  ...
  async function toggleSavedReport(report: SavedReport) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Report ${report.active ? 'paused' : 'activated'}.`)
    } finally {
      setReportBusy(false)
    }
  }

  async function setSavedReportCadence(
    report: SavedReport,
    cadence: SavedReport['cadence'],
  ) {
    if (reportBusy) return
    setReportBusy(true)
    try {
      await updateSavedReport(report.id, { cadence })
      setUiNotice(cadence === 'none' ? 'Report unscheduled.' : `Report scheduled ${cadence}.`)
    } finally {
      setReportBusy(false)
    }
  }

  async function saveReportRecipients(report: SavedReport) {
    if (reportBusy) return
    const recipients = (reportRecipientDrafts[report.id] ?? report.recipients.join(', '))
      .split(/[,;\n]/)
      .map((recipient) => recipient.trim())
      .filter(Boolean)
    setReportBusy(true)
    try {
      const saved = await updateSavedReport(report.id, { recipients })
      if (saved) {
        setReportRecipientDrafts((current) => {
          const next = { ...current }
          delete next[report.id]
          return next
        })
        setUiNotice('Report recipients updated.')
      }
    } finally {
      setReportBusy(false)
    }
  ...
  async function handleCreateServiceAppointment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Service appointment scheduled.')
      }
    } finally {
  ...
  async function setAppointmentStatus(appointment: ServiceAppointment, status: string) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Appointment marked ${status.replace(/_/g, ' ')}.`)
    } finally {
      setAppointmentBusy(false)
    }
  }
  ...
  async function reassignAppointmentTechnician(appointment: ServiceAppointment, technicianId: string) {
    ...
    try {
      ...
      if (saved) setUiNotice('Appointment reassigned.')
    } finally {
      setAppointmentBusy(false)
    }
  }
  ...
  async function handleCreateKnowledgeArticle(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setUiNotice('Knowledge article created.')
      }
    } finally {
  ...
  async function setArticleStatus(article: KnowledgeArticle, status: KnowledgeArticle['status']) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Article moved to ${status}.`)
    } finally {
      setArticleBusy(false)
    }
  }
  ...
  async function handleCreateForumTopic(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (saved) {
        ...
        setForumOpen(false)
        setUiNotice('Discussion topic created.')
      }
    } finally {
      setForumBusy(false)
    }
  ...
  async function setForumTopicStatus(topic: DiscussionTopic, status: string) {
    ...
    try {
      ...
      if (saved) setUiNotice(`Topic marked ${status}.`)
    } finally {
      setForumBusy(false)
    }
  }
  ...
  async function toggleForumTopicPin(topic: DiscussionTopic) {
    ...
    try {
      ...
      if (saved) setUiNotice(topic.pinned ? 'Topic unpinned.' : 'Topic pinned.')
    } finally {
      setForumBusy(false)
    }
  }
  ...
  async function toggleForumTopicComments(topic: DiscussionTopic) {
    ...
    try {
      ...
      setUiNotice(error instanceof Error ? error.message : 'Could not load comments.')
    } finally {
  ...
  async function handleAddForumComment(event: FormEvent<HTMLFormElement>, topic: DiscussionTopic) {
    ...
    try {
      ...
      setTopicComments(comments)
      setCommentDraft('')
      setUiNotice('Reply posted.')
      await refreshBackend()
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Reply failed.')
    } finally {
      setTopicCommentsBusy(false)
    }
  }
  ...
  async function handleCreateCustomer(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const saved = await createCustomer({
        ...
        job_title: customerDraft.jobTitle.trim(),
        location: customerDraft.location.trim(),
        timezone: customerDraft.timezone,
        language: customerDraft.language,
        contact_points: customerDraft.phone.trim()
          ? [{ channel: 'voice', value: customerDraft.phone.trim(), verified: false }]
          : [],
      ...
      if (saved) {
        ...
        setCustomerListRevision((value) => value + 1)
        setUiNotice('Customer created.')
      }
    } finally {
  ...
  function openCustomerEditor() {
    setCustomerEdit({
      ...
      companyId:
        backendSnapshot?.customers.find((customer) => customer.id === selectedCustomer.id)?.company_id ?? '',
      jobTitle: selectedCustomer.title === 'Customer' ? '' : selectedCustomer.title,
  ...
  async function handleUpdateCustomer(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const saved = await updateCustomer(selectedCustomer.id, {
        name,
        email,
        company_id: customerEdit.companyId || null,
        job_title: customerEdit.jobTitle.trim(),
        location: customerEdit.location.trim(),
        timezone: customerEdit.timezone,
        language: customerEdit.language,
        sentiment: customerEdit.sentiment,
        contact_points: (() => {
          const raw = backendSnapshot?.customers.find((customer) => customer.id === selectedCustomer.id)
          const withoutPhone = (raw?.contact_points ?? []).filter(
            (point) => !['voice', 'phone'].includes(point.channel),
          )
          return customerEdit.phone.trim()
            ? [
                ...withoutPhone,
                { channel: 'voice' as const, value: customerEdit.phone.trim(), verified: false },
              ]
            : withoutPhone
        })(),
      ...
      if (saved) {
        ...
        setCustomerListRevision((value) => value + 1)
        setUiNotice('Customer profile updated.')
      }
    } finally {
      setCustomerBusy(false)
    }
  ...
  function customerListQuery(): BackendCustomerListQuery {
    const since = new Date()
    let createdSince: string | undefined
    if (customerCreatedFilter === '24h') since.setDate(since.getDate() - 1)
    else if (customerCreatedFilter === '7d') since.setDate(since.getDate() - 7)
    else if (customerCreatedFilter === '30d') since.setDate(since.getDate() - 30)
    else createdSince = undefined
    if (customerCreatedFilter !== 'any') createdSince = since.toISOString()
    return {
      q: state.filters.search.trim() || undefined,
      company_id: customerCompanyFilter || undefined,
      tag: customerTagFilter.trim() || undefined,
  ...
  async function handleCustomerExport() {
    if (!backendSession || customerActionBusy) return
    setCustomerActionBusy(true)
    try {
      const payload = await exportBackendCustomers(customerListQuery(), backendSession)
      ...
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
      setUiNotice('Contacts exported to CSV.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Contact export failed.')
    } finally {
      setCustomerActionBusy(false)
    }
  }

  async function handleCustomerImport(file: File) {
    if (!backendSession || customerActionBusy) return
    setCustomerActionBusy(true)
    try {
      const result = await importBackendCustomersCsv(await file.text(), backendSession)
      await refreshBackend()
      setCustomerListRevision((value) => value + 1)
      setUiNotice(`Imported ${result.created} contact(s); ${result.skipped} row(s) skipped.`)
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Contact import failed.')
    } finally {
      setCustomerActionBusy(false)
  ...
  async function handleDeleteCustomer(customerId = selectedCustomer.id) {
    if (!backendSession || customerActionBusy) return
    setCustomerActionBusy(true)
    try {
      await deleteBackendCustomer(customerId, backendSession)
      await refreshBackend()
      setSelectedCustomerIds((current) => current.filter((id) => id !== customerId))
      setCustomerDeleteConfirmOpen(false)
      setCustomerListRevision((value) => value + 1)
      selectScreen('customers')
      setUiNotice('Contact deleted.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Contact could not be deleted.')
    } finally {
      setCustomerActionBusy(false)
    }
  }
  ...
  async function handleBulkCustomerDelete() {
    if (!backendSession || customerActionBusy || selectedCustomerIds.length === 0) return
    setCustomerActionBusy(true)
    let deleted = 0
    try {
      ...
      setCustomerListRevision((value) => value + 1)
      setUiNotice(`${deleted} contact(s) deleted.`)
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Bulk delete did not complete.')
    } finally {
      setCustomerActionBusy(false)
    }
  }
  ...
  async function handleMergeCustomer() {
    if (!backendSession || customerActionBusy || !customerMergeTargetId) return
    setCustomerActionBusy(true)
    try {
      await mergeBackendCustomers(selectedCustomer.id, customerMergeTargetId, backendSession)
      await refreshBackend()
      setCustomerMergeOpen(false)
      setCustomerListRevision((value) => value + 1)
      selectCustomer(customerMergeTargetId)
      setUiNotice('Contacts merged. Tickets and history now belong to the surviving contact.')
    } catch (error) {
      setUiNotice(error instanceof Error ? error.message : 'Contacts could not be merged.')
    } finally {
      setCustomerActionBusy(false)
    }
  }
  ...
  /** Humanise a wait in minutes: 45m, 3.5h, 12d. */
  ...
  async function handleTicketAction(actionId: (typeof freshdeskTicketActionItems)[number]['id']) {
    if (actionId === 'reply') {
    ...
    // Watch state is server-owned so it follows the operator across devices.
    const ticketId = selectedConversation.id
    const watching = watchedTicketIds.includes(ticketId)
    if (!backendSession || !online) {
      announcePrototype('Connect to the Omni backend before changing ticket watches.')
      return
    }
    try {
      await watchBackendTicket(ticketId, !watching, backendSession)
      ...
      announcePrototype(error instanceof Error ? error.message : 'Ticket watch could not be changed.')
    }
  }

  function openDirectChannel(channelId: ChannelId) {
    leaveLiveChatConversation()
  ...
  function sendLiveChatReply(conversation: OmniConversation, channelId: ChannelId) {
    ...
    liveTypingActiveRef.current = false
    submitComposer({
  ...
  function updateLiveChatDraft(conversationId: string, value: string) {
    const previousConversationId = liveTypingConversationRef.current
    if (previousConversationId && previousConversationId !== conversationId) {
      sendTyping(previousConversationId, false)
      liveTypingActiveRef.current = false
    }
    liveTypingConversationRef.current = conversationId
    setLiveChatDraft(value)

    const typing = value.trim().length > 0
    window.clearTimeout(liveTypingTimerRef.current)
    if (typing && !liveTypingActiveRef.current) {
      liveTypingActiveRef.current = sendTyping(conversationId, true)
    } else if (!typing && liveTypingActiveRef.current) {
      sendTyping(conversationId, false)
      liveTypingActiveRef.current = false
    }
    if (typing) {
      liveTypingTimerRef.current = window.setTimeout(() => {
        sendTyping(conversationId, false)
        liveTypingActiveRef.current = false
      }, 1_500)
    }
  ...
  function leaveLiveChatConversation(conversationId?: string) {
    window.clearTimeout(liveTypingTimerRef.current)
    const activeConversationId = conversationId || liveTypingConversationRef.current
    if (activeConversationId && liveTypingActiveRef.current) {
      sendTyping(activeConversationId, false)
    }
    liveTypingConversationRef.current = ''
    liveTypingActiveRef.current = false
    setLiveChatDraft('')
  }
          ...
          {freshdeskTicketActionItems.map(({ id, label, shortcut, icon: ActionIcon }) => {
            ...
            return (
              <button
                ...
                onClick={() => void handleTicketAction(id)}
              >
                <ActionIcon size={15} />
            ...
            {ticketDetailTab === 'time' ? (
              <div className="ticket-time-logs">
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    event.preventDefault()
                    void addTimeLog(selectedConversation.id)
                  }}
                >
                  <input
                    type="number"
                    ...
                    ticketTimeLogs.map((log) => (
                      <li key={log.id}>
                        ...
                        <button
                          ...
                          onClick={() => void removeTimeLog(selectedConversation.id, log.id)}
                        >
                          <X size={12} />
                        </button>
                      </li>
  ...
  function renderFreshchatModulePanel() {
    ...
    function moduleBody() {
      ...
      if (channelConsoleView === 'campaigns') {
        return (
          <Suspense fallback={<p className="report-catalog-state">Loading campaigns...</p>}>
            <CampaignConsole
              session={backendSession}
              canManage={Boolean(
                backendSession && userHasPermission(backendSession.user, 'supervisor.control'),
              )}
              onOpenSettings={() => {
                selectScreen('admin')
                openSetupModule('Provider credentials')
              }}
            />
          </Suspense>
        )
      ...
      if (channelConsoleView === 'people') {
        return (
          <Suspense fallback={<p className="report-catalog-state">Loading people...</p>}>
            <PeopleConsole
              session={backendSession}
              canManage={Boolean(
                backendSession && userHasPermission(backendSession.user, 'supervisor.control'),
              ...
              onOpenContacts={() => selectScreen('customers')}
            />
          </Suspense>
        )
      }
      if (channelConsoleView === 'reports') {
        return (
          <>
            <Suspense fallback={<p className="report-catalog-state">Loading reports...</p>}>
              <ReportCatalog
                session={backendSession}
                exporting={reportExportBusy}
                onExport={handleReportExport}
              />
              {backendSession && userHasPermission(backendSession.user, 'supervisor.control') ? (
                <ReportDeliveryHistory session={backendSession} />
              ) : null}
            </Suspense>
      ...
      if (channelConsoleView === 'settings') {
        return (
          <Suspense fallback={<p className="report-catalog-state">Loading Omnichat settings...</p>}>
            <OmnichatSettingsConsole
              session={backendSession}
              canManage={Boolean(
                backendSession && userHasPermission(backendSession.user, 'setup.manage'),
              )}
              onOpenAdmin={() => selectScreen('admin')}
            />
          </Suspense>
        )
      }
      return (
        <Suspense fallback={<p className="report-catalog-state">Loading AI Studio...</p>}>
          <AiStudioConsole
            session={backendSession}
            canManage={Boolean(
              backendSession && userHasPermission(backendSession.user, 'setup.manage'),
  ...
  function renderDirectChannelChat() {
    ...
    const remoteTyping = (typingByConversation[liveConversation?.id ?? ''] ?? []).filter(
      (actor) => actor.userId !== backendSession?.user.id,
    )
    const remoteReaders = (readsByConversation[liveConversation?.id ?? ''] ?? []).filter(
      (actor) => actor.userId !== backendSession?.user.id,
    )
    const latestRemoteReader = [...remoteReaders].sort(
      (a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime(),
    )[0]
    const realtimeLabel =
      omnichatConnectionStatus === 'connected'
        ? 'Live updates'
        : omnichatConnectionStatus === 'reconnecting'
          ? 'Reconnecting'
          : omnichatConnectionStatus === 'connecting'
            ? 'Connecting'
            : 'Updates offline'
    ...
    return (
      <section className="panel direct-chat-panel">
        <div className="panel-head">
          ...
          <span className={`realtime-connection ${omnichatConnectionStatus}`} role="status">
            {omnichatConnectionStatus === 'connected' ? <Wifi size={14} /> : <WifiOff size={14} />}
            {realtimeLabel}
          </span>
        </div>
            ...
            {channelConversations.map((conversation) => {
              ...
              return (
                <a
                  ...
                  onClick={(event) =>
                    handleAppLink(event, () => {
                      leaveLiveChatConversation(liveConversation?.id)
                      openConversation(conversation)
                    })
                  }
              ...
              <>
                  ...
                  <div>
                    ...
                    {remoteTyping.length > 0 ? (
                      <small className="typing-indicator">
                        {remoteTyping.map((actor) => actor.userName).join(', ')} typing...
                      </small>
                    ) : (
                      <small>
                        {liveConversation.subject} · {liveAgent?.name}
                        {latestRemoteReader ? ` · Viewed by ${latestRemoteReader.userName}` : ''}
                      </small>
                    )}
                  </div>
                ...
                <div className="direct-chat-composer">
                  <textarea
                    value={liveChatDraft}
                    onChange={(event) =>
                      updateLiveChatDraft(liveConversation.id, event.target.value)
                    }
  ...
  function renderCustomers() {
    ...
    const profileById = new Map(state.customers.map((customer) => [customer.id, customer]))
    const directoryRows = customerPageData.items
      .filter((customer) => !customer.tags.includes('production-readiness'))
      .map((customer) => ({ raw: customer, profile: profileById.get(customer.id) }))
      .filter((row): row is { raw: BackendCustomer; profile: typeof selectedCustomer } => Boolean(row.profile))
    const currentIds = directoryRows.map((row) => row.raw.id)
    const allCurrentSelected =
      currentIds.length > 0 && currentIds.every((id) => selectedCustomerIds.includes(id))
    const start = customerPageData.total === 0 ? 0 : customerPageData.offset + 1
    const end = Math.min(customerPageData.offset + customerPageData.returned, customerPageData.total)
    const lastPage = Math.max(0, Math.ceil(customerPageData.total / CUSTOMER_PAGE_SIZE) - 1)
    const rawCustomer =
      customerWorkspace?.customer ??
      backendSnapshot?.customers.find((customer) => customer.id === selectedCustomer.id)
    const contactCompany =
      customerWorkspace?.company ??
      (rawCustomer?.company_id ? companyById.get(rawCustomer.company_id) ?? null : null)
    const contactPhone =
      rawCustomer?.contact_points.find((point) => ['voice', 'phone'].includes(point.channel))?.value ??
      selectedCustomer.phone
    const contactVerified =
      rawCustomer?.contact_points.some((point) => point.verified) ?? false
    const customerTickets = customerWorkspace?.tickets ?? []
    const timelineEntries = customerTickets
      .flatMap((context) =>
        context.timeline.map((event) => ({ event, ticket: context.ticket })),
      )
      .sort((a, b) => b.event.created_at.localeCompare(a.event.created_at))
    const noteEntries = timelineEntries.filter(({ event }) => !event.public || event.type.includes('note'))
    const directory = state.customers.filter(
      (customer) => !customer.tags.includes('production-readiness'),
    )
    const selectedDirectoryIndex = directory.findIndex((customer) => customer.id === selectedCustomer.id)
    const previousCustomer = selectedDirectoryIndex > 0 ? directory[selectedDirectoryIndex - 1] : undefined
    const nextCustomer =
      selectedDirectoryIndex >= 0 && selectedDirectoryIndex < directory.length - 1
        ? directory[selectedDirectoryIndex + 1]
        : undefined

    const customerFormDialog = customerFormOpen ? (
      <div className="modal-backdrop" role="presentation" onMouseDown={() => setCustomerFormOpen(false)}>
        <form
          className="modal-card contact-form-dialog"
        ...
        >
          <div className="modal-head">
            ...
            <button className="icon-button" type="button" aria-label="Close new contact" onClick={() => setCustomerFormOpen(false)}>
              <X size={16} />
            </button>
          </div>
          <div className="contact-form-grid">
            <label>
              <span>Full name</span>
              <input required value={customerDraft.name} onChange={(event) => setCustomerDraft((current) => ({ ...current, name: event.target.value }))} />
            </label>
            <label>
              <span>Email address</span>
              <input required type="email" value={customerDraft.email} onChange={(event) => setCustomerDraft((current) => ({ ...current, email: event.target.value }))} />
            </label>
            <label>
              <span>Title</span>
              <input value={customerDraft.jobTitle} onChange={(event) => setCustomerDraft((current) => ({ ...current, jobTitle: event.target.value }))} />
            </label>
            <label>
              <span>Company</span>
              <select value={customerDraft.companyId} onChange={(event) => setCustomerDraft((current) => ({ ...current, companyId: event.target.value }))}>
            ...
            <label>
              <span>Location</span>
              <input value={customerDraft.location} onChange={(event) => setCustomerDraft((current) => ({ ...current, location: event.target.value }))} />
            </label>
            <label>
              <span>Time zone</span>
              <input value={customerDraft.timezone} onChange={(event) => setCustomerDraft((current) => ({ ...current, timezone: event.target.value }))} />
            </label>
            <label>
              <span>Language</span>
              <input value={customerDraft.language} onChange={(event) => setCustomerDraft((current) => ({ ...current, language: event.target.value }))} />
            </label>
            <label className="contact-form-span">
              <span>Tags</span>
              <input value={customerDraft.tags} onChange={(event) => setCustomerDraft((current) => ({ ...current, tags: event.target.value }))} placeholder="vip, corporate" />
            </label>
            <label className="contact-form-span">
              <span>Notes</span>
          ...
          <div className="modal-actions">
            <button type="button" onClick={() => setCustomerFormOpen(false)}>Cancel</button>
            <button className="primary-action" type="submit" disabled={customerBusy || !customerDraft.name.trim() || !customerDraft.email.trim()}>
              Create contact
            </button>
          </div>
        </form>
    ...
    const customerEditDialog = customerEditOpen ? (
      <div className="modal-backdrop" role="presentation" onMouseDown={() => setCustomerEditOpen(false)}>
        <form className="modal-card contact-form-dialog" aria-label="Edit contact" onSubmit={handleUpdateCustomer} onMouseDown={(event) => event.stopPropagation()}>
          <div className="modal-head">
            <div><span>Contact</span><h2>Edit {selectedCustomer.name}</h2></div>
            <button className="icon-button" type="button" aria-label="Close edit contact" onClick={() => setCustomerEditOpen(false)}><X size={16} /></button>
          </div>
          <div className="contact-form-grid">
            <label><span>Full name</span><input required value={customerEdit.name} onChange={(event) => setCustomerEdit((current) => ({ ...current, name: event.target.value }))} /></label>
            <label><span>Email address</span><input required type="email" value={customerEdit.email} onChange={(event) => setCustomerEdit((current) => ({ ...current, email: event.target.value }))} /></label>
            <label><span>Title</span><input value={customerEdit.jobTitle} onChange={(event) => setCustomerEdit((current) => ({ ...current, jobTitle: event.target.value }))} /></label>
            <label><span>Company</span><select value={customerEdit.companyId} onChange={(event) => setCustomerEdit((current) => ({ ...current, companyId: event.target.value }))}><option value="">No company</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label>
            <label><span>Work phone</span><input value={customerEdit.phone} onChange={(event) => setCustomerEdit((current) => ({ ...current, phone: event.target.value }))} /></label>
            <label><span>Location</span><input value={customerEdit.location} onChange={(event) => setCustomerEdit((current) => ({ ...current, location: event.target.value }))} /></label>
            <label><span>Time zone</span><input value={customerEdit.timezone} onChange={(event) => setCustomerEdit((current) => ({ ...current, timezone: event.target.value }))} /></label>
            <label><span>Language</span><input value={customerEdit.language} onChange={(event) => setCustomerEdit((current) => ({ ...current, language: event.target.value }))} /></label>
            <label><span>Sentiment</span><select value={customerEdit.sentiment} onChange={(event) => setCustomerEdit((current) => ({ ...current, sentiment: event.target.value as BackendCustomer['sentiment'] }))}>{(['positive', 'neutral', 'frustrated', 'angry'] as const).map((sentiment) => <option key={sentiment} value={sentiment}>{titleCase(sentiment)}</option>)}</select></label>
            <label><span>Tags</span><input value={customerEdit.tags} onChange={(event) => setCustomerEdit((current) => ({ ...current, tags: event.target.value }))} /></label>
            <label className="contact-form-span"><span>Notes</span><textarea rows={3} value={customerEdit.notes} onChange={(event) => setCustomerEdit((current) => ({ ...current, notes: event.target.value }))} placeholder="Replace the stored note" /></label>
          </div>
          <div className="modal-actions"><button type="button" onClick={() => setCustomerEditOpen(false)}>Cancel</button><button className="primary-action" type="submit" disabled={customerBusy}>Save changes</button></div>
        </form>
      </div>
    ) : null

    if (customerDetailRouteActive) {
      return (
        <div className="contact-detail-shell" data-testid="contact-detail-workspace">
          <div className="contact-detail-commandbar">
            <div className="contact-command-actions">
              <button type="button" onClick={openCustomerEditor}><Pencil size={15} /> Edit</button>
              <button type="button" onClick={() => setCustomerDeleteConfirmOpen(true)}><Trash2 size={15} /> Delete</button>
              <button type="button" onClick={() => { setCustomerMergeTargetId(''); setCustomerMergeOpen(true) }}><GitMerge size={15} /> Merge</button>
              <button type="button" onClick={() => setUiNotice('Customer portal identity assumption is unavailable until portal SSO is configured.')}><UserCheck size={15} /> Assume identity</button>
            </div>
            <div className="contact-record-navigation">
              <button type="button" aria-label="Previous contact" disabled={!previousCustomer} onClick={() => previousCustomer && selectCustomer(previousCustomer.id)}><ChevronLeft size={16} /></button>
              <button type="button" aria-label="Next contact" disabled={!nextCustomer} onClick={() => nextCustomer && selectCustomer(nextCustomer.id)}><ChevronRight size={16} /></button>
              <button type="button" aria-label={customerInfoCollapsed ? 'Show contact information' : 'Collapse contact information'} onClick={() => setCustomerInfoCollapsed((value) => !value)}><ExternalLink size={16} /></button>
            </div>
          </div>
          <div className={`contact-record-layout ${customerInfoCollapsed ? 'info-collapsed' : ''}`}>
            <section className="contact-record-main">
              <div className="contact-profile-summary">
                <div className="contact-profile-avatar"><UserRound size={48} /></div>
                <div className="contact-profile-heading">
                  <span className={`contact-verification ${contactVerified ? 'verified' : ''}`}>{contactVerified ? 'Verified' : 'Unverified'}</span>
                  <h2>{selectedCustomer.name}</h2>
                  <p>{selectedCustomer.email}</p>
                  <small>{selectedCustomer.title}{contactCompany ? ` · ${contactCompany.name}` : ''}</small>
                </div>
                <div className="contact-profile-commands">
                  <button className="primary-action" type="button" onClick={() => openQuickCreate('email')}><Plus size={15} /> New ticket</button>
                  {contactPhone ? <a className="button-link" href={`tel:${contactPhone.replace(/\s/g, '')}`}><Phone size={15} /> Call</a> : <button type="button" disabled><Phone size={15} /> Call</button>}
                </div>
              </div>
              <div className="contact-tabs" role="tablist" aria-label="Contact activity">
                {(['timeline', 'tickets', 'notes', 'forums'] as CustomerDetailTab[]).map((tab) => (
                  <button key={tab} type="button" role="tab" aria-selected={customerDetailTab === tab} className={customerDetailTab === tab ? 'active' : ''} onClick={() => setCustomerDetailTab(tab)}>{titleCase(tab)}</button>
                ))}
              </div>
              <div className="contact-tab-panel" role="tabpanel">
                {customerWorkspaceBusy ? <div className="contact-empty-state"><RefreshCw className="spin" size={24} /><strong>Loading contact history</strong></div> : null}
                {!customerWorkspaceBusy && customerDetailTab === 'timeline' ? (
                  timelineEntries.length ? <div className="contact-timeline">{timelineEntries.map(({ event, ticket }) => (
                    <article key={event.id} className="contact-timeline-entry">
                      <div className="contact-timeline-icon">{event.public ? <MessageSquare size={15} /> : <StickyNote size={15} />}</div>
                      <div><div className="contact-timeline-meta"><strong>{event.actor}</strong><span>{formatTime(event.created_at)}</span></div><p>{event.body}</p><button type="button" onClick={() => selectConversation(ticket.id)}>{ticket.public_id} · {ticket.subject}</button></div>
                    </article>
                  ))}</div> : <div className="contact-empty-state"><MessageSquare size={34} /><strong>No conversations yet</strong><span>New contact activity will appear here.</span></div>
                ) : null}
                {!customerWorkspaceBusy && customerDetailTab === 'tickets' ? (
                  customerTickets.length ? <div className="contact-ticket-list">{customerTickets.map((context) => <button key={context.ticket.id} type="button" onClick={() => selectConversation(context.ticket.id)}><span className={`status-dot status-${context.ticket.status}`} /><span><strong>{context.ticket.subject}</strong><small>{context.ticket.public_id} · {titleCase(context.ticket.status)} · {formatTime(context.ticket.updated_at)}</small></span><ChevronRight size={16} /></button>)}</div> : <div className="contact-empty-state"><Inbox size={34} /><strong>No tickets for this contact</strong></div>
                ) : null}
                {!customerWorkspaceBusy && customerDetailTab === 'notes' ? (
                  <div className="contact-notes-list">{selectedCustomer.notes ? <article><StickyNote size={16} /><div><strong>Contact note</strong><p>{selectedCustomer.notes}</p></div></article> : null}{noteEntries.map(({ event }) => <article key={event.id}><StickyNote size={16} /><div><strong>{event.actor}</strong><p>{event.body}</p><small>{formatTime(event.created_at)}</small></div></article>)}{!selectedCustomer.notes && noteEntries.length === 0 ? <div className="contact-empty-state"><StickyNote size={34} /><strong>No notes yet</strong><button type="button" onClick={openCustomerEditor}>Add a contact note</button></div> : null}</div>
                ) : null}
                {!customerWorkspaceBusy && customerDetailTab === 'forums' ? <div className="contact-empty-state"><MessageCircle size={34} /><strong>No forum activity</strong><span>Forum posts attributed to this contact will appear here.</span></div> : null}
              </div>
            </section>
            {!customerInfoCollapsed ? <aside className="contact-info-panel">
              <section><div className="contact-info-heading"><Users size={16} /><strong>Contact info</strong><ChevronDown size={15} /></div><dl>
                <div><dt>Tags</dt><dd>{selectedCustomer.tags.length ? <span className="contact-tag-list">{selectedCustomer.tags.map((tag) => <span key={tag}>{tag}</span>)}</span> : 'Add tags'}</dd></div>
                <div><dt>Email</dt><dd><a href={`mailto:${selectedCustomer.email}`}>{selectedCustomer.email}</a></dd></div>
                <div><dt>Work phone</dt><dd>{contactPhone || '—'}</dd></div>
                <div><dt>Company</dt><dd>{contactCompany?.name ?? '—'}</dd></div>
                <div><dt>Title</dt><dd>{rawCustomer?.job_title || '—'}</dd></div>
                <div><dt>Location</dt><dd>{rawCustomer?.location || '—'}</dd></div>
                <div><dt>Time zone</dt><dd>{rawCustomer?.timezone || '—'}</dd></div>
                <div><dt>Language</dt><dd>{rawCustomer?.language || '—'}</dd></div>
                <div><dt>Created</dt><dd>{rawCustomer?.created_at ? formatTime(rawCustomer.created_at) : '—'}</dd></div>
              </dl></section>
              <section className="contact-todo"><div className="contact-info-heading"><ClipboardList size={16} /><strong>To-do</strong><ChevronDown size={15} /></div><div className="contact-todo-body"><div className="contact-todo-input"><input value={todoDraft} onChange={(event) => setTodoDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void addTodo() }} placeholder="Add a to-do" aria-label="Add a to-do" /><button type="button" aria-label="Add to-do" onClick={() => void addTodo()} disabled={!todoDraft.trim()}><Plus size={15} /></button></div>{todos.length ? todos.map((todo) => <label key={todo.id}><input type="checkbox" checked={todo.done} onChange={() => void toggleTodo(todo.id)} /><span>{todo.label}</span><button type="button" aria-label={`Delete ${todo.label}`} onClick={() => void removeTodo(todo.id)}><X size={13} /></button></label>) : <p>No tasks to do.</p>}</div></section>
            </aside> : null}
          </div>
          {customerEditDialog}
          {customerMergeOpen ? <div className="modal-backdrop" role="presentation" onMouseDown={() => setCustomerMergeOpen(false)}><form className="modal-card compact-modal" aria-label="Merge contact" onSubmit={(event) => { event.preventDefault(); void handleMergeCustomer() }} onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span>Merge contact</span><h2>Choose the surviving contact</h2></div><button className="icon-button" type="button" aria-label="Close merge contact" onClick={() => setCustomerMergeOpen(false)}><X size={16} /></button></div><p>Tickets, conversations, identities, consent records, and history from <strong>{selectedCustomer.name}</strong> will move to the selected contact.</p><label><span>Surviving contact</span><select required value={customerMergeTargetId} onChange={(event) => setCustomerMergeTargetId(event.target.value)}><option value="">Select a contact</option>{directory.filter((customer) => customer.id !== selectedCustomer.id).map((customer) => <option key={customer.id} value={customer.id}>{customer.name} · {customer.email}</option>)}</select></label><div className="modal-actions"><button type="button" onClick={() => setCustomerMergeOpen(false)}>Cancel</button><button className="primary-action" type="submit" disabled={!customerMergeTargetId || customerActionBusy}>Merge contacts</button></div></form></div> : null}
          {customerDeleteConfirmOpen ? <div className="modal-backdrop" role="presentation" onMouseDown={() => setCustomerDeleteConfirmOpen(false)}><div className="modal-card compact-modal" role="dialog" aria-modal="true" aria-label="Delete contact" onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span>Delete contact</span><h2>Delete {selectedCustomer.name}?</h2></div></div><p>The contact will be removed from active views. Existing operational records remain retained for audit.</p><div className="modal-actions"><button type="button" onClick={() => setCustomerDeleteConfirmOpen(false)}>Cancel</button><button className="danger-action" type="button" onClick={() => void handleDeleteCustomer()} disabled={customerActionBusy}>Delete contact</button></div></div></div> : null}
        </div>
      )
    }

    return (
      <div className="contact-list-shell" data-testid="contact-list-workspace">
        <div className="contact-list-titlebar">
          <div className="contact-view-switcher">
            <button type="button" aria-expanded={customerViewsOpen} onClick={() => setCustomerViewsOpen((value) => !value)}><Users size={15} /> Contact Views <ChevronDown size={14} /></button>
            {customerViewsOpen ? <div className="contact-view-menu" role="menu"><button type="button" role="menuitem" onClick={() => { setCustomerPage(0); setCustomerCreatedFilter('any'); setCustomerTagFilter(''); setCustomerViewsOpen(false) }}>All contacts</button><button type="button" role="menuitem" onClick={() => { setCustomerPage(0); setCustomerCreatedFilter('7d'); setCustomerViewsOpen(false) }}>Recently added</button><button type="button" role="menuitem" onClick={() => { setCustomerPage(0); setCustomerTagFilter('vip'); setCustomerViewsOpen(false) }}>VIP contacts</button></div> : null}
          </div>
          <strong>All contacts</strong>
          <span className="contact-count-badge">{customerPageData.total.toLocaleString()}</span>
        </div>
        <div className="contact-list-toolbar">
          <label className="contact-select-all"><input type="checkbox" checked={allCurrentSelected} onChange={() => setSelectedCustomerIds((current) => allCurrentSelected ? current.filter((id) => !currentIds.includes(id)) : Array.from(new Set([...current, ...currentIds])))} /><span className="sr-only">Select all contacts on this page</span></label>
          <label className="contact-list-search"><Search size={15} /><input value={state.filters.search} onChange={(event) => { setCustomerPage(0); setFilters({ search: event.target.value }) }} placeholder="Search all contacts" aria-label="Search all contacts" /></label>
          <input ref={customerImportInputRef} className="visually-hidden-file" type="file" accept=".csv,text/csv" aria-label="Import contacts CSV" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleCustomerImport(file) }} />
          <button type="button" onClick={() => void handleCustomerExport()} disabled={customerActionBusy}><Download size={15} /> Export</button>
          <button type="button" onClick={() => customerImportInputRef.current?.click()} disabled={customerActionBusy}><Upload size={15} /> Import</button>
          <button className="primary-action" type="button" onClick={() => setCustomerFormOpen(true)}><Plus size={15} /> New contact</button>
          <span className="contact-page-count">{start} - {end} of {customerPageData.total.toLocaleString()}</span>
          <button className="icon-button" type="button" aria-label="Previous contacts page" disabled={customerPage === 0} onClick={() => setCustomerPage((value) => Math.max(0, value - 1))}><ChevronLeft size={16} /></button>
          <button className="icon-button" type="button" aria-label="Next contacts page" disabled={customerPage >= lastPage} onClick={() => setCustomerPage((value) => Math.min(lastPage, value + 1))}><ChevronRight size={16} /></button>
          <button type="button" aria-pressed={customerFiltersOpen} onClick={() => setCustomerFiltersOpen((value) => !value)}><Filter size={15} /> {customerFiltersOpen ? 'Hide filters' : 'Show filters'}</button>
        </div>
        {selectedCustomerIds.length ? <div className="contact-bulk-toolbar"><strong>{selectedCustomerIds.length} selected</strong><button type="button" onClick={() => void handleCustomerExport()}><Download size={14} /> Export</button><button type="button" onClick={() => void handleBulkCustomerDelete()} disabled={customerActionBusy}><Trash2 size={14} /> Delete</button><button type="button" onClick={() => setSelectedCustomerIds([])}>Clear</button></div> : null}
        <div className={`contact-list-layout ${customerFiltersOpen ? 'filters-open' : ''}`}>
          <div className="contact-table-wrap" tabIndex={0} role="region" aria-label="Contacts table">
            <table className="contact-table">
              <thead><tr><th><span className="sr-only">Select</span></th><th>Contact</th><th>Title</th><th>Company</th><th>Email address</th><th>Work phone</th><th>Facebook</th><th>Twitter</th><th><span className="sr-only">Actions</span></th></tr></thead>
              <tbody>
                {customerListBusy && directoryRows.length === 0 ? <tr><td colSpan={9}><div className="contact-empty-state"><RefreshCw className="spin" size={22} /><strong>Loading contacts</strong></div></td></tr> : null}
                {!customerListBusy && directoryRows.length === 0 ? <tr><td colSpan={9}><div className="contact-empty-state"><Users size={30} /><strong>No contacts found</strong><span>Change the filters or add a new contact.</span></div></td></tr> : null}
                {directoryRows.map(({ raw, profile }) => {
                  const company = raw.company_id ? companyById.get(raw.company_id) : undefined
                  const phone = raw.contact_points.find((point) => ['voice', 'phone'].includes(point.channel))?.value ?? ''
                  const facebook = raw.contact_points.find((point) => point.channel === 'facebook')?.value ?? ''
                  const twitter = raw.contact_points.find((point) => point.channel === 'twitter')?.value ?? ''
                  const verified = raw.contact_points.some((point) => point.verified)
                  return <tr key={raw.id}>
                    <td><input type="checkbox" aria-label={`Select ${raw.name}`} checked={selectedCustomerIds.includes(raw.id)} onChange={() => setSelectedCustomerIds((current) => current.includes(raw.id) ? current.filter((id) => id !== raw.id) : [...current, raw.id])} /></td>
                    <td><a className="contact-name-cell" href={routeHref({ screen: 'customers', customer: raw.id })} onClick={(event) => handleAppLink(event, () => selectCustomer(raw.id))}><span className="contact-avatar">{initials(raw.name)}</span><span><strong>{raw.name}</strong><small>{verified ? 'Verified' : 'Unverified'}</small></span></a></td>
                    <td>{raw.job_title || '—'}</td><td>{company?.name ?? '—'}</td><td><a href={`mailto:${raw.email}`}>{raw.email}</a></td><td>{phone ? <a href={`tel:${phone.replace(/\s/g, '')}`}>{phone}</a> : '—'}</td><td>{facebook || '—'}</td><td>{twitter || '—'}</td>
                    <td><button className="icon-button" type="button" aria-label={`Open ${profile.name}`} onClick={() => selectCustomer(raw.id)}><MoreHorizontal size={16} /></button></td>
                  </tr>
                })}
              </tbody>
            </table>
          </div>
          {customerFiltersOpen ? <aside className="contact-filter-panel"><div className="contact-filter-heading"><strong>Filters</strong><button type="button" onClick={() => { setCustomerPage(0); setCustomerCompanyFilter(''); setCustomerTagFilter(''); setCustomerTimezoneFilter(''); setCustomerCreatedFilter('any') }}>Clear all</button></div><label><span>Created</span><select value={customerCreatedFilter} onChange={(event) => { setCustomerPage(0); setCustomerCreatedFilter(event.target.value) }}><option value="any">Any time</option><option value="24h">Last 24 hours</option><option value="7d">Last 7 days</option><option value="30d">Last 30 days</option></select></label><label><span>Time zone</span><select value={customerTimezoneFilter} onChange={(event) => { setCustomerPage(0); setCustomerTimezoneFilter(event.target.value) }}><option value="">Any time zone</option><option value="Africa/Lagos">Africa/Lagos</option><option value="Africa/Accra">Africa/Accra</option><option value="Europe/London">Europe/London</option></select></label><label><span>Tags</span><input value={customerTagFilter} onChange={(event) => { setCustomerPage(0); setCustomerTagFilter(event.target.value) }} placeholder="Any tag" /></label><label><span>Companies</span><select value={customerCompanyFilter} onChange={(event) => { setCustomerPage(0); setCustomerCompanyFilter(event.target.value) }}><option value="">Any company</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><button className="primary-action" type="button" onClick={() => { setCustomerPage(0); setCustomerListRevision((value) => value + 1) }}>Apply</button></aside> : null}
        </div>
        {customerFormDialog}
      </div>
    )
  ...
  function renderAutomation() {
    return (
      <Suspense fallback={<div className="route-loading">Loading workflows…</div>}>
        <WorkflowsWorkspace
          rules={state.rules}
          slaPolicies={state.slaPolicies}
          onToggleRule={toggleRule}
          onCreateRule={async (input: BackendCreateAutomationRuleInput) => {
            if (!backendSession) throw new Error('Sign in to create an automation rule.')
            await createBackendAutomationRule(input, backendSession)
            await refreshBackend()
            setUiNotice('Automation rule created.')
          }}
          onUpdateRule={async (ruleId, input) => {
            if (!backendSession) throw new Error('Sign in to update an automation rule.')
            await patchBackendAutomationRule(ruleId, input, backendSession)
            await refreshBackend()
            setUiNotice('Automation rule updated.')
          }}
        />
      </Suspense>
    )
  ...
  function renderForums() {
    return (
      <Suspense fallback={<div className="route-loading">Loading forums…</div>}>
        <ForumsWorkspace
          topics={state.discussionTopics}
          currentUser={backendSession?.user.name ?? backendSession?.user.email ?? ''}
          onCreate={async (input) => Boolean(await createDiscussionTopic(input))}
          onUpdate={async (topicId, input) => Boolean(await updateDiscussionTopic(topicId, input))}
          onLoadComments={async (topicId) => {
            if (!backendSession) throw new Error('Sign in to read forum replies.')
            return fetchBackendDiscussionComments(topicId, backendSession)
          }}
          onReply={async (topicId, body) => {
            if (!backendSession) throw new Error('Sign in to reply to a topic.')
            await createBackendDiscussionComment(topicId, { body }, backendSession)
            await refreshBackend()
          }}
        />
      </Suspense>
  ...
  function renderAiAgents() {
    return (
      <Suspense fallback={<div className="route-loading">Loading AI Agents…</div>}>
        <AiAgentsWorkspace
          session={backendSession}
          canManage={Boolean(
            backendSession && userHasPermission(backendSession.user, 'setup.manage'),
  ...
  function renderCampaigns() {
    return (
      <Suspense fallback={<div className="route-loading">Loading campaigns…</div>}>
        <CampaignsWorkspace
          session={backendSession}
          canManage={Boolean(
            backendSession && userHasPermission(backendSession.user, 'supervisor.control'),
  ...
  function renderCustomObjects() {
    return (
      <Suspense fallback={<div className="route-loading">Loading custom objects…</div>}>
        <CustomObjectsWorkspace
          objects={state.customObjects}
          canManage={Boolean(
            backendSession && userHasPermission(backendSession.user, 'setup.manage'),
          )}
          onCreate={async (input) => Boolean(await createCustomObject({
            key: input.key,
            name: input.name,
          ...
          onUpdate={async (objectId, input) => Boolean(await updateCustomObject(objectId, {
            name: input.name,
            description: input.description,
            fields: input.fields?.map((field) => ({
              key: field.key,
  ...
  function renderAnalytics() {
    ...
    return (
      <div className="management-grid">
        <section className="panel span-2 analytics-library-panel" aria-label="Analytics report library">
          <div className="panel-head">
            <div>
              <span>Analytics</span>
              <h2>Reports</h2>
          ...
          <div className="analytics-library-shell">
            <aside aria-label="Report views">
              <label><Search size={15} /><input aria-label="Search reports" placeholder="Search" value={analyticsReportSearch} onChange={(event) => setAnalyticsReportSearch(event.target.value)} /></label>
              <span>Library</span>
              {[
                { id: 'catalog' as AnalyticsReportGroup, label: 'All reports' },
                { id: 'saved' as AnalyticsReportGroup, label: 'My reports' },
                { id: 'scheduled' as AnalyticsReportGroup, label: 'Scheduled reports' },
              ].map((tab) => (
                <button
                  type="button"
                  key={tab.id}
                  aria-pressed={analyticsReportGroup === tab.id}
              ...
              <span>Workspace</span>
              <button type="button" disabled title="Available when report sharing is configured">Shared reports <small>Not configured</small></button>
              <button type="button" disabled title="Available when report favorites are configured">Favorites <small>Not configured</small></button>
            </aside>
            <div className="analytics-library-main">
              <header>
                <h3>{analyticsReportGroup === 'catalog' ? 'All reports' : analyticsReportGroup === 'saved' ? 'My reports' : 'Scheduled reports'}</h3>
                <label>Sort by: <select aria-label="Sort reports" value={analyticsReportSort} onChange={(event) => setAnalyticsReportSort(event.target.value as 'modified' | 'name')}><option value="modified">Last modified date</option><option value="name">Name</option></select></label>
              </header>
              {analyticsReportGroup === 'catalog' ? (
                <Suspense fallback={<p className="report-catalog-state">Loading reports...</p>}>
                  <ReportCatalog
                    session={backendSession}
                    exporting={reportExportBusy}
                    search={analyticsReportSearch}
                    sort={analyticsReportSort}
                    onExport={handleReportExport}
                  />
                </Suspense>
              ) : (
                ...
                <div className="canned-form-row">
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) => setReportDraft((current) => ({
                        ...current,
                        reportType: event.target.value as BackendReportType,
                      }))}
                      disabled={reportBusy}
                    >
                  ...
                  <label>
                    ...
                    <input
                      ...
                      placeholder="quality@wakanow.com"
                      required={analyticsReportGroup === 'scheduled'}
                      disabled={reportBusy}
                    />
                  </label>
              ...
              <div className="report-card-grid">
                  ...
                  .map((report) => (
                    <article key={report.id} className={report.active ? '' : 'inactive'}>
                      ...
                      <div className="report-card-actions">
                        <button
                          type="button"
                          disabled={!backendSession || reportExportBusy !== null}
                          onClick={() => void handleReportExport(report.reportType as BackendReportType)}
                        >
                          <Download size={13} />
                          Export CSV
                        </button>
                        <label className="report-cadence">
                          ...
                          <select
                            ...
                            onChange={(event) => void setSavedReportCadence(report, event.target.value as SavedReport['cadence'])}
                            disabled={reportBusy}
                          >
                        ...
                        <label className="report-recipients-control">
                          <span>Recipients</span>
                          <input
                            type="text"
                            value={reportRecipientDrafts[report.id] ?? report.recipients.join(', ')}
                            onChange={(event) => setReportRecipientDrafts((current) => ({
                              ...current,
                        ...
                        <button
                          type="button"
                          disabled={reportBusy || reportRecipientDrafts[report.id] === undefined}
                          onClick={() => void saveReportRecipients(report)}
                        >
                          Save recipients
                        </button>
                ...
                {state.savedReports.filter((report) =>
                  ...
                  {analyticsReportGroup === 'scheduled' ? (
                    <Suspense fallback={<p className="report-catalog-state">Loading deliveries...</p>}>
                      <ReportDeliveryHistory session={backendSession} />
                    </Suspense>
                  ) : null}
                </>
        ...
        <section className="panel span-2" id="analytics-report-detail">
          <div className="panel-head">
            <div>
              <span>Reporting</span>
  ...
  function renderLegacyWorkforce() {
  ...
  function renderWorkforce() {
    return (
      <Suspense fallback={<div className="omnichat-gate" aria-busy="true">Loading scheduling workspace</div>}>
        <SchedulingWorkspace
          appointments={state.serviceAppointments}
          technicians={state.agents}
          customers={state.customers}
          onCreate={async (input: SchedulingCreateInput) => {
            const saved = await createServiceAppointment({
              title: input.title,
            ...
            if (saved) setUiNotice('Service task scheduled.')
            return Boolean(saved)
          }}
          onUpdate={async (appointmentId: string, input: SchedulingUpdateInput) => {
            const saved = await updateServiceAppointment(appointmentId, {
              title: input.title,
            ...
            if (saved) setUiNotice('Service task updated.')
            return Boolean(saved)
          }}
        />
      </Suspense>
      ...
      <div className="management-grid">
        ...
        >
          <div className="panel-head">
            <div>
              <span>Setup</span>
              <h2>{activeSetupTool ? activeSetupTool : 'Admin settings'}</h2>
            </div>
            <ShieldCheck size={20} />
          </div>
          {activeSetupTool ? <div className="setup-dashboard">
          ...
          {activeSetupTool ? (
            ...
            <>
            <nav className="setup-category-nav" aria-label="Admin categories">
              {setupSectionOptions.map((section) => (
                <button key={section.id} type="button" onClick={() => document.getElementById(`setup-group-${section.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>
                  <strong>{section.label}</strong>
                  <span>{setupSectionDescriptions[section.id]}</span>
                </button>
              ))}
            </nav>
            <div className="setup-tool-grid" role="region" aria-label="Setup tools">
              {setupSectionOptions.map((section) => {
                ...
                return (
                  <div className="setup-tool-group" id={`setup-group-${section.id}`} key={section.id}>
                    <h3>{section.label}</h3>
                    {categories.map((category) => (
          ...
          <Suspense fallback={<p className="report-catalog-state">Loading migration controls...</p>}>
            <ImportControlPlane session={backendSession} />
          </Suspense>
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  if (saved) {
                                    ...
                                    setUiNotice(`Temporary password reset for ${user.name}.`)
                                  }
                                } finally {
                                  setUserActionBusy(false)
                                }
                ...
                <article key={message.id}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setUiNotice('Outbound retry requested.')
                    }}
                  >
                    <RefreshCw size={15} />
                    Retry
  ...
  function renderScreen() {
    ...
    if (state.selectedScreen === 'forums') return renderForums()
    if (state.selectedScreen === 'ai-agents') return renderAiAgents()
    if (state.selectedScreen === 'campaigns') return renderCampaigns()
    if (state.selectedScreen === 'custom-objects') return renderCustomObjects()
  ...
  if (state.selectedScreen === 'channels') {
    return (
      <Suspense fallback={<main className="omnichat-gate" aria-busy="true">Loading Omnichat workspace</main>}>
        <OmnichatWorkspace
          session={backendSession}
          online={online}
          onNavigate={(screen) => selectScreen(screen as ScreenId)}
          onSwitchMarket={(marketId) => switchMarket(marketId)}
  ...
  if (state.selectedScreen === 'inbox') {
    return (
      <Suspense fallback={<main className="omnichat-gate" aria-busy="true">Loading ticket workspace</main>}>
        <TicketWorkspace
          session={backendSession}
          online={online}
          onNavigate={(screen) => selectScreen(screen as ScreenId)}
  ...
  if (state.selectedScreen === 'knowledge') {
    return (
      <Suspense fallback={<main className="omnichat-gate" aria-busy="true">Loading knowledge workspace</main>}>
        <KnowledgeWorkspace
          session={backendSession}
          online={online}
          onNavigate={(screen) => selectScreen(screen as ScreenId)}
      ...
      <main className="main-shell">
        {!announcementDismissed ? (
          <div className="omni-ai-announcement" aria-label="Omni AI announcement">
            ...
            <button type="button" onClick={() => selectScreen('ai-agents')}>Explore AI Agent</button>
            <i />
        ...
        {uiNotice && <div className="app-toast">{uiNotice}</div>}
