...
const stateKey = 'state-v21'
const legacyBackendSessionKey = 'omni-ticket-backend-session'
...
function mergeReferenceData(state: OmniState): OmniState {
  return {
    ...
    tags: state.tags ?? [],
    csatSurveys: state.csatSurveys ?? [],
    emailNotifications: state.emailNotifications ?? [],
    scenarioAutomations: state.scenarioAutomations ?? [],
    ...
    cases: state.cases ?? [],
    responseMacros: state.responseMacros ?? [],
    epics: [],
    backlog: [],
    issues: [],
  }
...
function mapChannel(channel: BackendChannel): Channel {
  ...
  const presentation = channelPresentation[channelId]
  const healthScore = channel.health === 'healthy' ? 92 : channel.health === 'degraded' ? 68 : 42
  return {
    id: channelId,
    label: presentation?.label ?? channel.name,
    shortLabel: presentation?.shortLabel ?? channel.name.slice(0, 10),
...
function mapCustomer(
  ...
  return {
    ...
    title: customer.job_title || (company ? `${company.tier} account` : 'Customer'),
...
function mergeBackendSnapshot(current: OmniState, snapshot: BackendSnapshot): OmniState {
  ...
  const mappedConversations = snapshot.tickets.map((context) => mapConversation(context, settings))
  const conversations =
    mappedConversations.length > 0 ? mappedConversations : initialOmniState.conversations
  const mappedChannels = snapshot.channels
    .map(mapChannel)
    .map((channel) => ({
      ...channel,
      avgWaitMinutes: channelAvgWaitMinutes(channel.id, conversations),
    }))
  const channels = mappedChannels.length > 0 ? mappedChannels : initialOmniState.channels
  const conversationsById = new Map(conversations.map((conversation) => [conversation.id, conversation]))
  const companiesById = new Map(snapshot.companies.map((company) => [company.id, company]))
  const mappedCustomers = snapshot.customers.map((customer) =>
    mapCustomer(customer, companiesById, snapshot.tickets, snapshot.csat_feedback ?? []),
  )
  const customers = mappedCustomers.length > 0 ? mappedCustomers : initialOmniState.customers
  ...
  const selectedCustomerId =
    customers.find(
      (customer) =>
        customer.id === current.selectedCustomerId && customer.id !== EMPTY_CUSTOMER_ID,
    )?.id ??
...
export function useOmniStore() {
  ...
  useEffect(() => {
    let cancelled = false
    window.localStorage.removeItem(legacyBackendSessionKey)
    readState()
      .then((stored) => {
        if (!cancelled) {
          const nextState =
  ...
  async function loginQaAdmin() {
    ...
    try {
      const session = await loginQaAdminBackend()
      await applyBackendSession(session)
    } catch (error) {
      saveBackendSession(null)
      setBackendSync((current) => ({
        ...current,
        status: 'error',
        error: error instanceof Error ? error.message : 'QA administrator login failed',
      }))
    }
  }
    ...
    restoreBackendSession()
      ...
      .catch((error) => {
        ...
        setBackendSync((current) => ({
          ...
          error: isBackendAuthError(error)
            ? undefined
  ...
  const filteredConversations = useMemo(() => {
    return state.conversations
      .filter((conversation) => conversation.id !== EMPTY_TICKET_ID)
      .filter((conversation) =>
  ...
  const metrics = useMemo(() => {
    ...
    const avgHealth = state.channels.length
      ? Math.round(
          state.channels.reduce((total, channel) => total + channel.health, 0) /
            state.channels.length,
        )
      : 0
    const avgOccupancy = state.agents.length
      ? Math.round(
          state.agents.reduce((total, agent) => total + agent.occupancy, 0) /
            state.agents.length,
        )
      : 0
    const csat = state.agents.length
      ? (
